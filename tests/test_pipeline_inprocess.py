...
def test_pipeline_inprocess_builds_all_three_deliverables(tmp_path, monkeypatch):
    ...
    # silent (proved in tests/test_arista.py). Coverage-honest on AJ: the all-Cisco fleet has no MLAG -> silent.
    ...
    # JSON dialect, the same coverage-honest contract. Silent on AJ (the all-Cisco fleet runs no SRX cluster).
    ...
    # beyond on-prem multi-vendor to PUBLIC CLOUD via the same offline JSON pattern. Silent on AJ (no cloud export).
    ...
    # firewall/HA construct through the same coverage-honest pipeline. Silent on AJ (no FortiGate captures).
