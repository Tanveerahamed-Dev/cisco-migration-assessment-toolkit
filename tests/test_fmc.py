...
_DEVS_HEALTHY = _wrap([
    {"name": "AJ-FTD-01", "hostName": "10.1.1.10", "model": "FTDv", "sw_version": "7.2.5",
     "isConnected": True, "healthStatus": "green", "deploymentStatus": "DEPLOYED"},
])
_DEVS_BAD = _wrap([
    {"name": "AJ-FTD-01", "hostName": "10.1.1.10", "model": "FTDv", "sw_version": "7.2.5",
     "isConnected": True, "healthStatus": "green", "deploymentStatus": "DEPLOYED"},
    {"name": "AJ-FTD-99", "hostName": "10.1.1.99", "model": "FTDv", "sw_version": "7.2.5",
...
def test_parse_fmc_devices_unwraps_items():
    ...
    assert len(d) == 2
    bad = [x for x in d if x["name"] == "AJ-FTD-99"][0]
...
def test_device_disconnected_fires():
    ...
    assert d is not None and "AJ-FTD-99" in str(d["evidence"]["summary"])
