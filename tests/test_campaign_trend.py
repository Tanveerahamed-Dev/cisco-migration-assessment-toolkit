...
def test_campaign_workbook_sheets(tmp_path):
    ...
    assert wb.sheetnames == [
        "Campaign Summary", "Timeline", "Burndown", "Protocol Adjacencies",
        "Adjacent Comparison Receipts",
    ]
    ...
    receipts = wb["Adjacent Comparison Receipts"]
    assert receipts.cell(2, 1).value == "PAIR SET STATUS"
    assert receipts.cell(2, 6).value == "NOT_VERIFIED"
    assert receipts.cell(2, 9).value is False
