...
def test_build_device_physical_emits_none_active_ports_when_status_unobserved(cp):
    ...
    status was not collected (e.g. AJ's DS17/AS01: chassis ports known, status uncollected) falsely
...
def test_endpoint_census_discloses_basis_vs_canonical_n_endpoints():
    ...
    on AJ) silently contradicted the Exec Summary headline (5127). The sheet must disclose its basis."""
    from openpyxl import Workbook
    import cisco_toolkit.excel as X
    from cisco_toolkit.model import InterfaceData
    def port(mode, mac):
