...
def test_golden_drift_ignores_banner_art_and_cli_preamble():
    """[audit-5 FF#8/#14] A 'banner motd ^C ... ^C' body (ASCII-art border, the '* ALJAZEERA... *' lines, the
    ...
    def cfg(site):
        return ("Building configuration...\n"
                ...
                "ip domain-name aljazeera.net\n")

    rc = {"sw1": cfg("DOHA"), "sw2": cfg("AJDOH"), "sw3": cfg("ARDOH")}
    base = [str(b).lower() for b in (compute_golden_drift(rc)["baseline"] or [])]
    joined = " ".join(base)
    # 'media network' is banner-art-only; 'aljazeera' alone would false-match the legit 'ip domain-name aljazeera.net'.
    ...
    assert "ip domain-name aljazeera.net" in base
