...
def test_custom_weight_changes_score(cp):
    ...
    cfg = cp.ScoringConfig(l1_weights={"single-fiber-uplink": 25})
...
def test_custom_cap_changes_capping(cp):
    ...
    cfg = cp.ScoringConfig(caps={"L1": 50, "L3": 30, "XL": 45, "PROTO": 25})
...
def test_custom_bands_change_label(cp):
    ...
    cfg = cp.ScoringConfig(bands=[(95, "Excellent", "x"), (0, "Critical", "y")])
...
def test_readiness_rule_is_configurable(cp):
    ...
    cfg = cp.ScoringConfig(readiness={**cp.SCORING.readiness, "gateway_redundancy": "warn"})
