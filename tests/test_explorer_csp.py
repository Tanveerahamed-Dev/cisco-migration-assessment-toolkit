...
def test_explorer_template_has_strict_no_egress_csp():
    csp = _csp(open(TEMPLATE, encoding="utf-8").read())
    assert csp, "explorer template must carry a Content-Security-Policy meta tag"
    d = _directives(csp)
    assert d.get("default-src") == "default-src 'none'"     # deny by default
    assert "'none'" in d.get("connect-src", "")             # no egress — the air-gapped doctrine
    assert "'none'" in d.get("object-src", "")              # no plugins
    assert "'none'" in d.get("base-uri", "")                # no <base> hijack
    assert "http:" not in csp and "https:" not in csp       # no external origin is ever allowed


def test_embedded_explorer_preserves_csp():
    """write_html_explorer injects the live snapshot but must NOT strip the head CSP."""
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert _csp(open(out, encoding="utf-8").read()), "emitted explorer lost its CSP"
