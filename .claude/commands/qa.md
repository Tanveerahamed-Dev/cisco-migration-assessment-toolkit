...
Then CLOSE THE LOOP, in the same turn (mandatory): save the reviewer's final message VERBATIM to a UTF-8 file (session scratchpad, e.g. `qa-verdict.md` — a file, never stdin: Windows consoles are cp1252 and mangle em-dashes) and append it to the quality scorecard with `python -m cisco_toolkit.scorecard --record <file>`. The command prints what it recorded; a file without the per-artifact verdict signature records nothing. This in-session step exists because under the Agent SDK harness the `SubagentStop` appender never fires and subagent transcripts are empty — skip it and the feedback nerve starves. Record the reviewer's message as-is; never paraphrase or summarize it into the file (proposer ≠ verifier).

Trust note (P0-6): the appender stamps the row with the judge's measured trust (`judge_tnr` from the latest judge-baseline row); while that baseline sits below `cisco_toolkit.scorecard.JUDGE_TNR_FLOOR`, an APPROVE is machine-marked `provisional: true` — **advisory, never a gate pass by itself** (semantics + policy: `docs/quality/README.md`). A BLOCK stands regardless — grounded counterexamples don't need a trusted judge.
