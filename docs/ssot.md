...
| **External advisory intel** — sanitized, signed PSIRT/advisory feed (Phase 5 "eyes", consumer side) | `docs/intel/feed-*.jsonl` (produced by the **gated** egress research lane; **none yet**) | `cisco_toolkit.intel_feed.load_feeds()` / `verify_feed()` — provenance-gated (sanitized + SHA-256 + forbidden-scan), never hand-edit | consumer + gate built (`tests/test_intel_feed.py`); **PRODUCER not wired — egress, gated (D2/D3/D4)**. Format contract in `docs/intel/README.md`. Empty → "no intel feed (gated)", never "no advisories" |
