...
| **Vault digest** — one-way, sanitized, read-only distilled domain facts for recall (D3/D4, ADR-0001 Amendment 1) | vault→repo digest (**gated: none yet** — needs the digest producer + Ollama) | (planned) local Ollama RAG, fused via RRF with graph+docs (D10); Rule-3-scrubbed via `research_lane/sanitize.py`, signed like the intel feed | **GATED** — ADR-0001 Amendment 1 grants the mechanism (one-way/sanitized/read-only digest, NOT raw pages); Ollama optional + gracefully-degrading (absent ⇒ recall falls back to graph+docs) |
