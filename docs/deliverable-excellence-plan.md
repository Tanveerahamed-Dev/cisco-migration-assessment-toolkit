...
> **Current status (reconciled 2026-08-07):** P0–P2 are shipped. P3's visible
> provenance controls are also shipped. The Excel File > Info metadata and the
> optional explorer-header enhancement remain deliberately deferred; neither is
> a correctness or release blocker. The initiative and its 10 Laws remain
> authoritative even though the implementation backlog below is now largely a
> historical record.
...
## P1 — Client-facing depth ✅ SHIPPED
- ✅ **Inputs-Required register:** `add_inputs_required` is wired into all seven DOCX writers. It
  includes the universal coverage-honest gaps and the mandatory completion/peer-review gate for every
  `<placeholder>`; CRD adds its generator-specific unconfirmed requirements through `extra_rows`.
- ✅ **MOP:** answer-first Executive Summary BLUF, explicit per-wave rollback triggers,
  pre-implementation checklists, and the communications/escalation plan all ship in `mop.py`.
- ✅ **CRD:** Constraints & Assumptions and Out of Scope ship in `crd.py`, with requirements grounded
  in the document's evidence and design context.

## P2 — Remaining deliverables ✅ SHIPPED
- ✅ **Architecture review:** the cover uses `ssot.canonical_facts(snap)['n_devices']`, and the document
  carries a named availability / single-device-blast-radius analysis.
- ✅ **Engagement:** `gate_record` drives decision-gate freshness, and the decision table includes
  Alternatives / rationale.
- ✅ **Operations:** Backup & Recovery and Known Issues & Operating Caveats sections are present.
- ✅ **Runbook:** the shared Inputs-Required register is wired in.
...
- **excel.py** — `wb.properties` (CoreProperties: creator/created/keywords=version — golden-safe metadata)
  would be set at the orchestrator's `wb.save` (`COLLECT_PARSE_V3_23_0.py:2464`); **CAUTION**: that's the flagship
