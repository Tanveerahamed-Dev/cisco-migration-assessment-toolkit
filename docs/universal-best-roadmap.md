...
> (`tests/test_fib.py`, 29 tests) and runtime-verified on the real [HISTORY-REDACTED] snapshot, then HARDENED by a 12-defect
...
- **Format-fidelity (recurring):** a regex-derived `line_no` citing the wrong line on a real format variant is a
  *confidently-wrong* claim — worse than none. → W3-2 byte-match verifier + tests on **real [HISTORY-REDACTED] collected text**.
