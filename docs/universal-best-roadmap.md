...
| W3-2 | **Precision-tiered evidence chips** (LINE/BLOCK/DEVICE/FLEET) on the live evidence dict — never fake-precise; ship with a **byte-match verifier** (demote-to-BLOCK on mismatch, tested vs real [HISTORY-REDACTED] text) | `causal.py:95-106` (`_flow(evidence=)`) → `CausalFlow.tsx:179` + explorer `:333`; tested `tests/test_causal_flows.py` | **L** |
