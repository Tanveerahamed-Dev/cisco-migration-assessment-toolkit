...
The AJ fleet runs no observed IPv6 SVIs in the canonical snapshot, so on AJ this detector correctly stays silent (coverage-honest) — it adds a new capability proven on the synthetic fixture (access1), exactly like _d_fhrp_resilience/_d_nve_peer_health were first proven off-AJ.
