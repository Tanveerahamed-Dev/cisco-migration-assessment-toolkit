...
The [HISTORY-REDACTED] fleet runs no observed IPv6 SVIs in the canonical snapshot, so on [HISTORY-REDACTED] this detector correctly stays silent (coverage-honest) — it adds a new capability proven on the synthetic fixture (access1), exactly like _d_fhrp_resilience/_d_nve_peer_health were first proven off-[HISTORY-REDACTED].
