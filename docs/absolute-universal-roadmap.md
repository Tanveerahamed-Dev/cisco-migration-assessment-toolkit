...
Baseline: **778 engine tests green**, canonical AJ fleet steady (the universality additions are coverage-honest
and silent on AJ).
...
| **Multicast** | PIM RP resilience; **RPF integrity ✅** ((S,G) Null IIF — #1 mcast outage; coverage-honest, excludes the benign (\*,G)-Null case, proven on AJ's 36 such entries) | **MSDP** liveness, **mrouter-aware** querier-gap |
...
5. **Non-disruptive.** The canonical AJ fleet's decision count is unchanged (the addition is silent on evidence
...
- **Verified:** `tests/test_arista.py` 15/15; **full suite 793 green** (incl. the global emit-invariant + all
  three coverage count-locks); golden diff = **+18 lines, arista-only** (purely additive); the real **AJ fleet
  steady at 42 decisions** with `arista` *not-observed* (zero AJ MLAG evidence → 0 new AJ decisions).
