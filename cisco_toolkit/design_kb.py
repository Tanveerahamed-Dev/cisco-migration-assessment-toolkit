...
# The placement / segmentation / flow-symmetry principles stay design doctrine (the L1-L4 assessment does
# not collect DMZ topology, micro-segmentation policy or path symmetry) -- HLD-narrative only. The HA
# principle below IS engine_actionable: the engine now reads 'show failover' (parse_asa_failover /
# build_firewall) and _d_firewall_ha_degraded fires on an enabled-but-broken pair (coverage-honest -- silent
# when no firewall / no failover is observed; a standalone firewall is never cry-wolfed as 'broken HA').
_FIREWALL_DESIGN_ADDENDUM = [
    ...
    {
        "id": "firewall-ha-failover-degraded",
        "title": "A stateful firewall in the path needs an operational HA peer -- config-present is not redundancy",
        "domain": "security",
        "priority": "High",
        "design_intent": "A stateful firewall on the data path is a single point of failure unless it has a "
        "working high-availability peer. Cisco ASA / Secure Firewall failover keeps a hot standby with "
        "synchronized connection state, so a unit/interface/power failure fails over in seconds without "
        "dropping established flows. The senior check is OPERATIONAL, not configurational: a pair can have "
        "'failover' fully configured yet sit with the standby Failed, Disabled or Not Detected -- so it has "
        "no real redundancy while every config read says 'configured'. Active + Standby-Ready is the only "
        "healthy resting state; the sync states (Negotiation/Cold Standby/Sync*/Bulk Sync) are normal only "
        "in passing.",
        "observable": "show failover: 'Failover On' with a This host/Other host unit state of Failed, "
        "Disabled or Not Detected (parse_asa_failover -> firewall.failover; detector _d_firewall_ha_degraded).",
        "trigger": "An enabled failover pair with a unit Failed/Disabled or the peer Not Detected -- the "
        "standby is not ready, so the firewall is a single point of failure in the data path.",
        "recommended_action": "Treat the degraded pair as a cutover blocker: confirm the failover/state link "
        "is up, both units run compatible image + licensing, interface monitoring is healthy and config sync "
        "has completed, then restore the standby to Standby Ready BEFORE the firewall is on the cutover path. "
        "Where a firewall is intentionally standalone, design and document the accepted single-point-of-"
        "failure (or add a peer) -- do not leave a half-built pair.",
        "alternatives": "Active/active (multi-context) or clustering for scale + HA (more complex state/flow "
        "management); a single firewall with an explicitly accepted-risk standalone posture for a "
        "low-criticality edge.",
        "tradeoffs": "An HA pair doubles firewall cost and adds state-sync / coordinated-upgrade complexity, "
        "but removes the in-path single point of failure -- usually mandatory for a perimeter or inter-zone "
        "enforcement point.",
        "citation": "Cisco Secure Firewall ASA HA / Failover configuration guide (the show failover state "
        "machine: Active / Standby Ready / Failed / Not Detected / Disabled); standard stateful-HA practice.",
        "engine_actionable": True,
    },
    {
        "id": "firewall-resource-exhaustion",
        "title": "Size the firewall above its OBSERVED peak -- a box denying or near its resource limits drops traffic",
        "domain": "security",
        "priority": "High",
        "design_intent": "A stateful firewall enforces hard limits on connections, translations (xlates), hosts "
        "and per-context/per-class capacity; when peak load reaches a Limit the box DENIES (drops) the excess. "
        "For a migration this is a first-class SIZING input: a firewall already denying at peak, or peaking near "
        "its limit, will inherit and worsen those drops on a like-for-like or consolidated replacement. Senior "
        "design sizes the target to the OBSERVED peak plus headroom (and growth), not the average, and re-checks "
        "configured connection/embryonic limits. The administrative-plane counters (SSH/ASDM failed logins) and "
        "rate-limit drops are NOT capacity faults and are excluded.",
        "observable": "show resource usage: a data-plane resource (Conns/Xlates/Hosts) with Denied > 0, or Peak "
        ">= 90% of a numeric Limit (parse_asa_resource_usage -> firewall.resource_usage; _d_firewall_resource_exhaustion).",
        "trigger": "A data-plane firewall resource denying connections/xlates at peak, or peaking near its "
        "configured limit -- the box is dropping (or about to drop) production traffic.",
        "recommended_action": "Size the replacement (or consolidated) firewall above the observed peak with "
        "headroom for growth AND failover (an HA pair must carry full load on one unit); re-evaluate per-context "
        "and per-class connection / embryonic limits; capture 'show resource usage' + 'show perfmon' at a real "
        "busy period as the sizing baseline and pin it into the NRFU acceptance.",
        "alternatives": "Raise the configured limits on the current platform if headroom exists (a stopgap, not "
        "a fix if the silicon is the bound); distribute load across an additional firewall / context.",
        "tradeoffs": "Sizing to peak + headroom costs more capacity up front, but a firewall that drops at peak "
        "is an availability incident a migration should resolve, not carry forward.",
        "citation": "Cisco ASA performance & resource-management docs (show resource usage Denied / Limit); "
        "Cisco Secure Firewall sizing / capacity guidance.",
        "engine_actionable": True,
    },
]
...
# on the (S,G) case (the benign (*,G) case is excluded -- proven on the Meridian reference fleet's 36 (*,G)-Null entries).
_MULTICAST_RPF_ADDENDUM = [
    {
        ...
        "shows a Null IIF for locally-joined / well-known / SSM groups (36 such benign entries on the Meridian reference fleet) "
...
# via the read-only Open API JSON channel (parse_ise_nodes / build_ise), NOT device 'show' text. All three are
# engine_actionable: the deployment-node export gates them, and they are coverage-honest (silent with no
# export) and standalone-aware (a Cisco-supported single-node deployment is never cry-wolfed as 'no redundancy').
# Sources: Cisco ISE Performance & Scalability / Deployment guides (PAN/MnT/PSN personas, node groups, max
# nodes), Cisco TrustSec & SD-Access design guides (ISE redundancy), Cisco ISE Open API (deployment node).
_ISE_DESIGN_ADDENDUM = [
    {
        "id": "ise-deployment-node-unreachable",
        "title": "Keep every ISE deployment node Connected to the Primary Admin -- a partitioned node serves no AAA",
        "domain": "security",
        "priority": "High",
        "design_intent": "An ISE deployment is a replicated cluster; the Primary Admin node (PAN) tracks each "
        "node's status. A node that is not 'Connected' has lost contact with the PAN -- it is not receiving "
        "policy/config replication and (if it is a Policy Service node) the RADIUS clients pointed at it will "
        "time out, dropping 802.1X/MAB endpoints to fail-auth. The check is operational: a node configured in "
        "the deployment but partitioned is invisible to a config-only view yet actively breaking authentication.",
        "observable": "Open API GET /api/v1/deployment/node: a node whose nodeStatus != 'Connected' "
        "(parse_ise_nodes -> ise.nodes[].node_status; detector _d_ise_node_unreachable). Transient in-progress/"
        "registering states are excluded; this is reachability, not a replication verdict.",
        "trigger": "A deployment node reported not Connected to the PAN -- partitioned / unreachable, serving "
        "no AAA or policy to the endpoints that depend on it.",
        "recommended_action": "Confirm the node is powered, on-net, time-synced (NTP) and that the PAN<->node "
        "path + certificates + replication are healthy; restore it to Connected before the access layer is cut "
        "over onto it. Ensure the switches' RADIUS server-groups list a reachable PSN so a single partition "
        "does not blackhole auth.",
        "alternatives": "Operate fewer, larger nodes (simpler, less resilient); distribute PSNs across sites "
        "behind RADIUS load-balancing (more resilient, more complex).",
        "tradeoffs": "Cluster health/replication adds operational surface, but a partitioned node silently "
        "breaks authentication for everything pointed at it.",
        "citation": "Cisco ISE Administration / Deployment guide (deployment node status, replication); Cisco "
        "ISE Open API (GET /api/v1/deployment/node).",
        "engine_actionable": True,
    },
    {
        "id": "ise-policy-service-node-redundancy",
        "title": "Deploy >=2 Policy Service Nodes (a node group) -- a single PSN is an authentication SPOF",
        "domain": "security",
        "priority": "High",
        "design_intent": "The Policy Service Node (PSN) is what actually answers RADIUS for 802.1X/MAB and "
        "serves the TrustSec environment-data / SGACL matrix switches download. With a single PSN, its loss "
        "or saturation drops every 802.1X/MAB port on every dependent switch to fail-auth / critical-VLAN -- "
        "an estate-wide outage from one node. Senior design puts >=2 PSNs behind the RADIUS clients (in a PSN "
        "node group for session-state sync) and points each switch's RADIUS server-group at both.",
        "observable": "Open API services[]: exactly ONE node carrying 'Session' in a deployment of >=2 nodes "
        "(parse_ise_nodes; detector _d_ise_psn_no_redundancy). The >=2-node gate excludes a standalone.",
        "trigger": "A multi-node ISE deployment with a single Policy Service node -- an authentication single "
        "point of failure for every switch that uses it for 802.1X/MAB/TrustSec.",
        "recommended_action": "Deploy at least two PSNs (a node group so session state survives a PSN loss); "
        "configure each switch's RADIUS server-group with both PSNs and dead-criteria/deadtime; size PSN count "
        "to the authenticating-endpoint scale. Confirm before cutover so the maintenance window cannot orphan "
        "the access layer.",
        "alternatives": "A single PSN with an accepted-risk posture only for a small/low-criticality site; "
        "RADIUS to a non-ISE fallback for break-glass (weaker policy).",
        "tradeoffs": "A second PSN (and node group) costs a node + sync overhead, but removes the single point "
        "of failure for all identity-based access.",
        "citation": "Cisco ISE Performance & Scalability / Deployment guide (PSN node groups, redundancy); "
        "Cisco TrustSec & SD-Access design guides (redundant PSN plane).",
        "engine_actionable": True,
    },
    {
        "id": "ise-admin-monitoring-redundancy",
        "title": "Give the ISE PAN and MnT a Secondary -- a single Admin/Monitoring node loses management or visibility",
        "domain": "security",
        "priority": "Medium",
        "design_intent": "The Primary Admin node (PAN) owns configuration and drives replication; the "
        "Monitoring node (MnT) owns authentication logging and reporting. A production deployment runs a "
        "Secondary of each so that losing the primary leaves the deployment manageable (the Secondary PAN can "
        "be promoted) and observable (the Secondary MnT keeps the auth logs you need to troubleshoot). A "
        "single PAN or single MnT means a node loss leaves you unable to change policy or blind to what is "
        "authenticating -- acute risk during a migration cutover.",
        "observable": "Open API roles[]: a deployment of >=2 nodes where a PrimaryAdmin is observed but no "
        "SecondaryAdmin (single PAN), or a PrimaryMonitoring but no SecondaryMonitoring (single MnT) "
        "(parse_ise_nodes; detector _d_ise_admin_monitoring_redundancy).",
        "trigger": "A multi-node ISE deployment missing a Secondary Admin and/or Secondary Monitoring node -- "
        "no management failover or no logging failover.",
        "recommended_action": "Add a Secondary PAN (config/replication failover + promotion path) and a "
        "Secondary MnT (logging/reporting failover) so the control and logging planes survive a node loss; "
        "verify promotion and log continuity as part of NRFU before relying on the deployment at cutover.",
        "alternatives": "A single combined PAN+MnT for a small deployment with an accepted-risk posture; "
        "external SIEM ingestion to reduce MnT-loss blindness (does not restore management).",
        "tradeoffs": "Secondary PAN/MnT nodes cost hardware + replication overhead, but a single management or "
        "logging node is a recovery and visibility gap exactly when a migration needs both.",
        "citation": "Cisco ISE Deployment / High Availability guide (PAN + MnT primary/secondary, promotion).",
        "engine_actionable": True,
    },
]
DOCTRINE.extend(_ISE_DESIGN_ADDENDUM)

# --------------------------------------------------------------------- Cisco Secure Firewall Mgmt Center (FMC)
# FMC manages an FTD fleet centrally; for an FMC-managed fleet the controller is the source of truth (often no
# device CLI). Assessed via the read-only REST JSON channel (parse_fmc_* / build_fmc). All engine_actionable
# (the per-endpoint export gates them) and coverage-honest (silent with no export; standalone FTD / standalone
# FMC never cry-wolfed; an operator-suspended 'Disabled' HA pair excluded). Sources: Cisco Secure Firewall
# Management Center REST API Quick Start (devicerecords, ftddevicehapairs, deployabledevices, fmchastatuses) +
# FTD HA / FMC HA configuration & deployment guides.
_FMC_DESIGN_ADDENDUM = [
    {
        "id": "ftd-ha-pair-degraded",
        "title": "An FTD HA pair must hold a real Active/Standby split -- FMC is the authoritative HA verdict",
        "domain": "security",
        "priority": "Critical",
        "design_intent": "A Firepower Threat Defense firewall on the data path is a single point of failure "
        "unless its high-availability pair holds a healthy Active/Standby split. For an FMC-managed fleet the "
        "controller -- not the device CLI -- is the authoritative view of HA state, so the assessment reads it "
        "from FMC. The check is operational: a pair can be configured yet sit with a node Failed / Not Detected "
        "or split (both Active), so it has no working failover while the config still says 'HA'.",
        "observable": "FMC ftddevicehapairs: a node currentStatus Failed / Not Detected, or both units the same "
        "role (parse_fmc_ha_pairs -> fmc.ha_pairs; detector _d_ftd_ha_degraded). 'Disabled' (operator-suspended) "
        "is excluded.",
        "trigger": "An FTD HA pair FMC reports with a node Failed/Not-Detected or split -- no working standby.",
        "recommended_action": "Treat the degraded pair as a cutover blocker: confirm the failover/state link, "
        "unit health and the HA config in FMC, restore the standby to Standby, and (if intentionally suspended "
        "for maintenance) re-enable HA before the firewall is on the cutover path.",
        "alternatives": "Clustering for scale + HA (more complex); an accepted-risk standalone FTD for a "
        "low-criticality edge (document the single point of failure).",
        "tradeoffs": "An HA pair doubles firewall cost and adds state-sync complexity, but removes the in-path "
        "single point of failure for a perimeter/inter-zone enforcement point.",
        "citation": "Cisco Secure Firewall Threat Defense HA + FMC REST API (ftddevicehapairs currentStatus: "
        "Active/Standby/Failed/Not Detected/Disabled); standard stateful-HA design practice.",
        "engine_actionable": True,
    },
    {
        "id": "fmc-device-disconnected",
        "title": "Every FMC-managed FTD must stay connected + healthy -- a disconnected device can't be deployed-to",
        "domain": "security",
        "priority": "High",
        "design_intent": "FMC manages, monitors and deploys policy to its FTD fleet over a management channel; "
        "a device that has lost that channel (or sits in a red health state) cannot be templated, deployed-to "
        "or monitored, and may be partitioned from management -- acute risk during a migration when policy is "
        "changing. The signal is the controller's own verdict (isConnected / healthStatus), not an inference.",
        "observable": "FMC devicerecords: isConnected==false, or healthStatus 'red' (parse_fmc_devices -> "
        "fmc.devices; detector _d_fmc_device_disconnected). green/yellow and the no-data black/blue stay silent.",
        "trigger": "A managed FTD FMC reports not-connected or red-health -- unmanaged and possibly partitioned.",
        "recommended_action": "Confirm the device is powered, on-net and not mid-reboot/deploy; restore the "
        "FMC<->FTD management channel and clear the health alert before counting the device in-scope at cutover.",
        "alternatives": "Out-of-band management path for resilience; local FDM management for a small site (loses "
        "central policy).",
        "tradeoffs": "A reliable management channel + health monitoring is operational overhead, but a device "
        "FMC can't reach is one you cannot safely change during a migration.",
        "citation": "Cisco Secure Firewall Management Center REST API (devicerecords isConnected / healthStatus).",
        "engine_actionable": True,
    },
    {
        "id": "fmc-deployment-pending",
        "title": "Reconcile staged-but-undeployed FMC changes before cutover -- running config must equal intent",
        "domain": "security",
        "priority": "Medium",
        "design_intent": "FMC separates intended config (staged in the manager) from running config (deployed to "
        "the device). A device with pending, undeployed changes is running a config that no longer equals FMC's "
        "intent. At migration time that drift is a hazard: deploying mid-cutover changes device behaviour at the "
        "worst moment, and not deploying leaves the documented intent un-applied -- either way the running state "
        "is not the known/intended one.",
        "observable": "FMC deployabledevices non-empty -- one or more devices with staged undeployed changes "
        "(parse_fmc_deployable -> fmc.deployable; detector _d_fmc_deployment_pending).",
        "trigger": "Managed FTD(s) with staged, undeployed configuration changes at assessment/cutover time.",
        "recommended_action": "Reconcile each pending deployment BEFORE the cutover window -- deploy and "
        "re-validate, or explicitly defer with sign-off -- so the firewall's running state is known and intended "
        "at cutover; capture the deployment state in the NRFU baseline.",
        "alternatives": "Freeze change windows ahead of the migration so no drift accumulates.",
        "tradeoffs": "Reconciling drift is pre-cutover effort, but cutting over a firewall whose running config "
        "differs from intent invites surprise behaviour during the change.",
        "citation": "Cisco Secure Firewall Management Center REST API (deployabledevices) + change-management practice.",
        "engine_actionable": True,
    },
    {
        "id": "fmc-manager-ha-degraded",
        "title": "Keep the FMC management plane redundant -- a degraded FMC HA leaves the fleet unmanageable",
        "domain": "security",
        "priority": "High",
        "design_intent": "The FMC is the single management plane for its FTD fleet; production deployments run an "
        "FMC HA pair so a manager loss does not strand the fleet. A Degraded / Split-Brain / unsynced FMC HA "
        "means there is no healthy standby manager -- losing the active FMC during a migration leaves the fleet "
        "unmanageable (no policy change, no deployment, no monitoring), and a split-brain risks divergent config.",
        "observable": "FMC fmchastatuses: haStatus Degraded / Split Brain, syncStatus not Synced, or peer "
        "unreachable (parse_fmc_ha_status -> fmc.ha_status; detector _d_fmc_manager_ha_degraded).",
        "trigger": "The FMC's own HA is degraded / split / unsynced -- no healthy standby manager.",
        "recommended_action": "Restore FMC HA synchronization and peer reachability (resolve any split-brain) "
        "before relying on the manager through the cutover; verify failover/promotion as part of NRFU.",
        "alternatives": "A single FMC with an accepted-risk posture for a small deployment (no manager failover); "
        "cloud-delivered management (cdFMC) to offload HA.",
        "tradeoffs": "An FMC HA pair costs a second appliance + sync overhead, but a single management plane is a "
        "recovery gap exactly when a migration needs to push and verify policy.",
        "citation": "Cisco Secure Firewall Management Center High Availability guide + REST API (fmchastatuses).",
        "engine_actionable": True,
    },
    {
        "id": "fmc-version-inversion",
        "title": "Keep the FMC at or ahead of every managed FTD's version -- a manager behind a device can't manage it",
        "domain": "security",
        "priority": "Medium",
        "design_intent": "Cisco's management compatibility rule is strict and directional: the FMC must run a "
        "software version equal to or NEWER than every Threat Defense device it manages. An FMC behind a managed "
        "FTD is an unsupported state -- the manager can fail to deploy policy to, or drive an upgrade of, the "
        "newer device. In a migration this surfaces when devices are upgraded ahead of the manager, or a new "
        "FTD is registered to an old FMC; the order of operations (FMC first, then devices) is the design rule.",
        "observable": "FMC serverVersion (info/serverversion) compared to each managed device's sw_version "
        "(devicerecords): FMC version < a device version (parse_fmc_server_version; _d_fmc_version_inversion). "
        "Version-compared, not date-based -- it does not rot.",
        "trigger": "The FMC runs an older software version than an FTD it manages -- an unsupported inversion.",
        "recommended_action": "Upgrade the FMC to at least the highest managed-FTD version (following the "
        "supported FMC->FTD upgrade path) BEFORE relying on central management through the migration; sequence "
        "the migration's upgrades manager-first, devices-second.",
        "alternatives": "Roll the over-advanced device back to a managed version (rarely desirable); split "
        "management so each FMC stays ahead of its own device set.",
        "tradeoffs": "Keeping the FMC ahead is a sequencing constraint on upgrades, but the inverse is an "
        "unsupported state that can block deployment/upgrade of the affected device.",
        "citation": "Cisco Secure Firewall Management Center / Threat Defense compatibility + upgrade guides "
        "(FMC version must be >= managed device version).",
        "engine_actionable": True,
    },
]
DOCTRINE.extend(_FMC_DESIGN_ADDENDUM)
...
# a fictional reference Solution Design (Nexus 9000 VXLAN BGP-EVPN Multi-Site fabric). Sources: ipSpace.net
...
# the fictional reference SDD/NRFU, and public IETF RFCs. Load-bearing standards facts were web-verified against
...
[
 ...
 },
 {
  ...
  "citation": "Meridian SDD Migration Approach ('only a single logical layer 2 connection per VLAN ... the EVPN VXLAN fabric is not forwarding any STP BPDUs'), 'Dot1Q VLAN and Spanning Tree Design', 'MAC Move Violation'",
  "engine_actionable": false
 },
 {
  ...
  "citation": "Meridian SDD 'Underlay Multicast (PIM-ASM & PIM Anycast RP)', 'Tenant Routed Multicast' ('considered for the future Deployment'), 'L3 TRM PIM RP Placement'; RFC 4610 (Anycast-RP) | TRM routed-multicast control plane is ngMVPN (RFC 6513/6514).",
  "engine_actionable": false
 },
 {
 ...
 },
 {
  ...
  "citation": "Meridian SDD 'Anycast Border Gateway' / 'BGW on Spine' / 'Route Server' / 'Next Hop unchanged' / 'Route-target rewrite'; corroborated by Cisco VXLAN EVPN Multi-Site design",
  "engine_actionable": false
 },
 {
 ...
 },
 {
  ...
  "citation": "Meridian SDD 'Shared Border' / 'Shared Border - Firewall' (Centralized Route Leaking, FW-Bypass-VRF) / 'Shared Border - Load Balancer'; ipSpace.net 'High-Speed Multi-Tenant Isolation' (transparent bump-in-the-wire vs routed insertion; routing adjacency as path check)",
  "engine_actionable": false
 },
 {
 ...
 },
 {
  ...
  "citation": "Meridian SDD 'DHCP RELAY' (Layer 2 Design) -- unique loopback as GiAddr; Option-82 (dhcp option vpn) scope selection by L2VNI",
  "engine_actionable": false
 },
 {
 ...
 },
 {
  ...
  "citation": "ipSpace.net DC Design Case Studies ch.6 'Designing a Private Cloud Network Infrastructure' (Management Network) (Pepelnjak); Meridian SDD 'Fabric Management' / 'Out-of-Band Management'; Cisco DC management/OOB best practice",
  "engine_actionable": false
 },
 {
...
# real Meridian reference snapshot: (1) rapid-PVST per-VLAN STP at high VLAN scale -> MST (reads protocol_health STP mode
...
_ACTIONABLE_DETECTOR_ADDENDUM_2 = [
    ...
    {
        "id": "dc-resolve-active-l2-instability-mac-flap",
        "title": "Resolve active L2 instability (MAC flapping) before freezing the baseline / cutover",
        "domain": "dc-switching",
        "priority": "High",
        "design_intent": "A MAC address the control plane logs flapping between two ports is, by definition, an "
        "ACTIVE Layer-2 problem in the LIVE network: the same host is being learned on two ports, which is "
        "almost always a forwarding loop (a redundant link STP failed to block, a mis-cabled patch), a "
        "dual-homed host bridging two access ports, or a unidirectional link. It is not a configuration smell "
        "to note for later -- it is happening now, and every flap re-floods and churns the MAC table for that "
        "VLAN. Migrating a domain that is actively flapping carries the loop (or the mis-cabling) straight into "
        "the target, where a fabric change can turn an intermittent flap into a broadcast storm. The "
        "instability must be found and cleared BEFORE the current-state baseline is frozen, because a flapping "
        "domain's baseline is not a stable reference to validate the cutover against.",
        "observable": "The engine's syslog_intelligence axis classifies %SW_MATM-4-MACFLAP_NOTIF (and the NX-OS "
        "L2FM equivalent) messages from the collected logs into per-host 'mac-flap' detections, each carrying "
        "the flapping VLAN and the two ports parsed from the log line.",
        "trigger": "snap['syslog_intelligence']['detections'] contains one or more kind=='mac-flap' entries (a "
        "MAC observed flapping between two ports within the collected log window).",
        "recommended_action": "For each flapping host, read the logged VLAN + the two ports, confirm STP state "
        "on both (a loop is a link that should be blocking but is forwarding -- resolve the root cause, do not "
        "just shut a port), check for a mis-cabled or dual-homed end host bridging two access ports, and rule "
        "out a unidirectional link (enable / verify UDLD, check the optic). Clear every active flap and confirm "
        "the domain is quiet for a sustained window BEFORE freezing its baseline and scheduling its cutover.",
        "alternatives": "Cut over with the flap unresolved and chase it after (cheapest now, but the target "
        "inherits the loop / mis-cabling and a fabric change can escalate it to a storm); port-disable the "
        "flapping port as a stop-gap (removes the symptom -- and possibly the host's connectivity -- not the "
        "cause).",
        "tradeoffs": "Diagnosing an active flap costs pre-cutover field / console time, but a flapping L2 domain "
        "is an unstable baseline and a live storm risk -- the only safe order is resolve, then baseline, then cut.",
        "citation": "Cisco MAC-flapping / %SW_MATM-4-MACFLAP_NOTIF troubleshooting (L2 loop, dual-homed host, "
        "unidirectional link); Cisco STP / UDLD loop-prevention design guidance.",
        "engine_actionable": True,
    },
    {
        "id": "routing-static-default-cutover-dependency",
        "title": "Re-point or dynamic-ize static default routes before any gateway/next-hop change (cutover black-hole)",
        "domain": "routing",
        "priority": "High",
        "design_intent": "A host whose default route (0.0.0.0/0) is STATIC has no dynamic reconvergence on its "
        "path of last resort: the route points at one configured next-hop and stays there regardless of what "
        "the network does. That is fine while the next-hop is stable, but a migration is precisely the moment "
        "the next-hop changes -- a new core / aggregation gateway, a re-addressed transit, a VRF or interface "
        "move. When it does, the static 0.0.0.0/0 still points at the OLD next-hop and silently black-holes ALL "
        "off-subnet traffic until someone re-points it by hand. A dynamically-learned default (OSPF/EIGRP/BGP) "
        "would simply reconverge. So every static default is an explicit, ordered cutover dependency: it must "
        "be re-pointed (or migrated to a dynamic / tracked default) as part of the change, not discovered "
        "broken after it.",
        "observable": "The engine's routes axis records, per host, the default route's source flag (s / s* "
        "static vs o/d/b dynamic) and its next-hop, parsed from 'show ip route'.",
        "trigger": "snap['routes'][host] has a 0.0.0.0/0 entry whose source is static (s / s*) with a configured "
        "next-hop (no dynamically-learned default to reconverge).",
        "recommended_action": "Inventory every static-default host and its next-hop; for each, decide the "
        "target default path and SEQUENCE its update into the cutover step for that device (or pre-stage a "
        "floating/tracked static + a dynamic default so it fails over instead of black-holing). Prefer "
        "migrating edge defaults to a dynamic protocol (or object-tracked static with IP SLA) so a future "
        "gateway change reconverges on its own. Validate reachability to the off-subnet world in NRFU after "
        "each re-point.",
        "alternatives": "Leave the static defaults and re-point them reactively during the window (cheapest to "
        "plan, but every missed host is a silent black-hole and a rollback trigger); blanket-convert to dynamic "
        "(cleanest long-term, but a larger change that needs its own validation).",
        "tradeoffs": "Re-pointing / dynamic-izing static defaults adds per-device cutover steps and NRFU "
        "checks, but it converts a class of silent post-cutover black-holes into a planned, verifiable step.",
        "citation": "Cisco routing design — a static default route does not reconverge; use a dynamic default, "
        "or an object-tracked / floating static (IP SLA) for gateway resilience; Cisco migration practice — "
        "re-point or dynamic-ize static next-hops before changing the gateway they point at.",
        "engine_actionable": True,
    },
]
...
# python-measured on the canonical Meridian reference snapshot, and adversarially refutation-gated. The emit-invariant
...
# surfaces this as a 'Media / timing fabric' TARGET-STATE DIMENSION (not a firing decision). Relevant to the Meridian
# (Meridian Media) broadcast estate; cited as a dimension driver. Web-verified (SMPTE ST 2059-2 / ST 2110-10).
...
# Universal-architecture KB principles (2026-06-23): the 26 detectors for ACI / VXLAN-EVPN /
# MPLS / SD-WAN / LISP / CTS / DMVPN / IPsec / BFD / IPv6 / CoPP / storm-control / QoS-runtime / FHRP
# fire on collected (non-Meridian) architecture state but PREVIOUSLY had no backing principle, so a fired
# decision rendered with the raw pid as title + empty citation/action. These principles (web-researched,
# primary-source-cited, independently citation-verified) complete them: each is engine_actionable=True
# and the design_advisor emit-invariant proves it emits when the architecture is present.
_UNIVERSAL_ARCH_ADDENDUM = json.loads('[{"id": "aci-critical-fault-raised", "title": "Clear or formally accept every CRITICAL APIC fault before freezing the migration baseline", "domain": "aci", "priority": "Critical", "design_intent": "In ACI the APIC raises a CRITICAL fault (severity \'critical\', the most severe of critical/major/minor/warning) when a managed object enters a condition the fabric considers service-impacting and self-healing has not resolved it. Faults are stateful and progress through a lifecycle (soaking, raised, raised-clearing, retaining); a fault that has reached the raised/critical state has soaked past its detection window, so it is a real, sustained condition, not a transient blip. Critical examples are concrete and serious: F0110 fltFabricNodeFailed (\'node not reachable\'), F1394 interface-physical-down on a fabric (leaf-to-spine) port, and the F0321/F0323/F0325 family for a diverged or degraded APIC cluster. Critical faults also flow down into the health-score computation, so an unaddressed critical is simultaneously dragging fabric, pod and node health. Operationally a raised critical means \'something the controller expects to be healthy is broken right now\' on the exact fabric you are about to migrate onto or off of.", "observable": "The aci axis records each APIC faultInst severity / lifecycle (lc) / acknowledgement (parse_aci_faults / moquery -c faultInst).", "trigger": "snap[\'aci\'][host].faults has a raised (lc=raised), unacknowledged (ack=no) fault at critical/major severity.", "recommended_action": "Work the fault by its lifecycle and its delegate chain, not by silencing it. (1) Enumerate critical faults at System > Faults and per-node under Fabric > Inventory > Pod > node > Faults; record the fault code, the affected MO (dn), and the lifecycle state. (2) Follow each fault to its ROOT MO \\u2014 a fault:Delegate surfaced on a parent points back to a fault:Inst on the object actually in trouble; hide delegated faults to find the true origin. (3) Look the code up in the ACI System Messages Reference and execute its documented remediation (e.g. F1394 -> verify the fabric port cabling/transceiver and that the peer is up; F0110 -> drive the node-not-active runbook; F0321/F0323/F0325 -> restore APIC cluster quorum/\'Fully Fit\'). (4) Remediate the underlying condition and confirm the fault transitions to raised-clearing then clears on its own. (5) Only after the real fix, if a residual is a known cosmetic/expected condition, ACKNOWLEDGE it (which deletes it immediately when retaining, or marks it expected otherwise) \\u2014 and document why. Re-run the fault sweep and require zero unexplained criticals before the cutover gate.", "alternatives": "The cheaper, riskier path is to acknowledge or squelch the fault to make the dashboard and the health score go green without fixing the cause \\u2014 acknowledging an ignored fault explicitly removes it from the health-score computation, and squelching a fault code discards all instances of it fabric-wide and from logs.", "tradeoffs": "Acknowledging/squelching takes seconds and unblocks the gate cosmetically, but it deletes your only early warning: the underlying defect (a dead fabric link, an unreachable node, a split cluster) then surfaces as a production outage during or after cutover with no fault left to point at it.", "citation": "Cisco APIC Faults, Events, and System Messages Management Guide \\u2014 Faults (severity levels, fault lifecycle soaking/raised/clearing/retaining, acknowledge vs. squelch, delegated faults, health-score impact); ACI System Messages Reference Guide \\u2014 ACI F0x Messages (e.g. F0110 fltFabricNodeFailed); Address ACI fault F1394 (interface-physical-down)", "engine_actionable": true}, {"id": "aci-fabric-health-degraded", "title": "Treat a degraded fabric health score as an aggregate alarm and drive the worst contributor to 100 before cutover", "domain": "aci", "priority": "Medium", "design_intent": "The ACI health score is a 0\\u2013100 rollup the APIC computes bottom-up: object and node health are derived from the weighted severity of the faults raised against each managed object, and the system/pod score is a weighted average of leaf health (normalized by learned endpoints) combined with a spine coefficient derived from the spines\' health. A score below the healthy threshold therefore is not a metric in itself \\u2014 it is a SYMPTOM that one or more faults of meaningful severity are raised somewhere in the tree, and the score will not recover until those faults clear. Because the score is endpoint-weighted, a degraded leaf carrying many endpoints (precisely the kind of access leaf a brownfield migration cuts traffic onto) depresses the fabric score the most, so \'degraded\' frequently localizes to exactly the device that matters for the move. A fabric you are migrating onto should be at or near 100; anything materially lower means the target\'s own assurance system is telling you it is not whole.", "observable": "The aci axis records the fabric-wide health score (parse_aci_health / moquery -c fabricHealthTotal).", "trigger": "snap[\'aci\'][host].health.cur < 90 (a measured low rollup score).", "recommended_action": "Decompose the score top-down to the offending object, then fix the fault \\u2014 never tune the threshold. (1) From the System/Pod dashboard, drill into the lowest-scoring pod, then the lowest-scoring node, to localize the drag. (2) On that node open Faults and rank by severity; the critical/major faults are what is pulling the number down. (3) Remediate the root-cause faults (this reuses the aci-critical-fault-raised runbook) and watch the node, pod and system scores climb as each clears. (4) Distinguish genuine degradation from health that is artificially suppressed \\u2014 confirm whether \'Ignore Acknowledged Faults\' / squelch / a Health Score Evaluation Policy is hiding faults, because an artificially HIGH score is as dangerous as a low one for a cutover decision. (5) Set and record a go/no-go health floor (e.g. fabric and each in-scope leaf at a defined high score with no unexplained critical/major) and gate the migration on it; capture the pre-cutover score as the baseline for the post-cutover diff.", "alternatives": "The cheaper, riskier alternative is to lower the acceptance bar \\u2014 declare the degraded score \'good enough\', or acknowledge/squelch the contributing faults so the number rises \\u2014 and proceed without curing the underlying condition.", "tradeoffs": "Relaxing the floor or suppressing contributors lets the gate pass immediately and avoids root-cause work, but you migrate onto a fabric whose own health model is flagging an unresolved defect, and you forfeit a trustworthy pre/post health baseline to prove the cutover did no harm.", "citation": "Cisco Application Centric Infrastructure Fundamentals \\u2014 Monitoring/Troubleshooting (system, pod and node health-score computation from per-object fault severity; endpoint-weighted leaf health and spine coefficient); Cisco APIC Faults guide (Ignore Acknowledged Faults / Health Score Evaluation Policies effect on the score)", "engine_actionable": true}, {"id": "aci-node-not-active", "title": "Restore any leaf/spine that is not Active to the registered Active state before relying on the fabric", "domain": "aci", "priority": "High", "design_intent": "A fabric node reaches the Active state only after the full discovery-and-registration sequence completes: a switch starts Undiscovered (manually pre-provisioned but not seen), becomes Discovered once it is cabled and the APIC sees it over LLDP on an uplink, and then moves to Active when it has a node ID, an assigned infra TEP address, and is registered to and reachable from the APIC cluster. A node that is NOT Active (stuck Discovering, sitting in Nodes Pending Registration with node ID 0, Inactive/unreachable, Disabled, or Unsupported) is not carrying production policy or data plane \\u2014 its EPGs, contracts, SVIs and uplinks are effectively absent from the fabric. In a brownfield migration this is critical twice over: if the not-Active node is a target leaf, the endpoints and VLANs you intend to cut to it have nowhere to land; if it is one half of a redundant leaf/spine pair, the fabric is silently running without redundancy and a single further failure during the window is an outage. The corresponding fault is concrete \\u2014 F0110 fltFabricNodeFailed, \'[node-failed][critical], node not reachable\'.", "observable": "The aci axis records each fabricNode fabric state (parse_aci_fabric_nodes / moquery -c fabricNode).", "trigger": "snap[\'aci\'][host].nodes has a node fabric_st decommissioned/inactive/disabled.", "recommended_action": "Walk the discovery state machine in order \\u2014 physical, then identity, then trust \\u2014 rather than re-registering blindly. (1) Confirm the node\'s current state in Fabric > Inventory > Fabric Membership / Nodes Pending Registration. (2) PHYSICAL: verify the inter-switch cabling and that the leaf port to the spine is an UPLINK (a downlink will not relay the spine\'s DHCP, a classic discovery blocker); check the LLDP neighbor with \'show lldp neighbors detail\' on the peer and confirm both sides run a supported, compatible image. (3) IDENTITY: from the leaf run \'acidiag avread\' (or \'avread\') to confirm it sees the APICs and an infra TEP from the configured TEP pool over the correct infra VLAN; if it is unregistered, register it with the intended node ID. (4) TRUST: on 4.2+ run \'show discoveryissues\' on the node to surface certificate/clock/connectivity failures, and validate the node certificate and APIC cluster reachability (\'Fully Fit\'). (5) Resolve the surfaced cause, confirm the node transitions to Active and its F0110/related faults clear, and only then admit it to the cutover scope. For a true replacement, follow the documented decommission/replace procedure so the old serial and TEP are cleanly reused.", "alternatives": "The cheaper, riskier alternative is to exclude the dead node from the cut and proceed on the survivors \\u2014 migrating onto the remaining Active leaf/spine and deferring the inactive one \\u2014 instead of fixing discovery first.", "tradeoffs": "Proceeding on the survivors keeps the schedule but converts a redundant pair into a single point of failure for the duration of the window, and if the not-Active node was a target, you simply have insufficient capacity/landing space and discover it mid-cutover.", "citation": "Troubleshoot ACI Fabric Discovery \\u2014 Initial Fabric Setup (node discovery states, uplink/LLDP/infra-VLAN/TEP/DHCP checks, acidiag avread, show discoveryissues, certificate validation); Cisco APIC Getting Started Guide \\u2014 Fabric Initialization and Switch Discovery (Undiscovered->Discovered->Active, registration); ACI System Messages \\u2014 F0110 fltFabricNodeFailed", "engine_actionable": true}, {"id": "aci-vrf-enforcement-unenforced", "title": "Resolve an Unenforced tenant VRF to an explicit, contract-based enforced policy before cutover (or record it as an accepted exception)", "domain": "aci", "priority": "Medium", "design_intent": "ACI\'s native model is an allow-list: a VRF defaults to Enforced, where intra-VRF EPG-to-EPG traffic is implicitly denied (an any-any deny) and communication requires an explicit contract. An UNENFORCED VRF inverts this to an any-any PERMIT \\u2014 every EPG in the VRF talks to every other EPG with no contract and no filtering. That is a deliberate, security-relevant posture, and detecting it on a fabric headed into migration is significant for three reasons. First, it usually signals that no segmentation design exists for that tenant yet, so the brownfield\'s east-west security is effectively flat. Second, Unenforced is a feature-blocker: with it set you CANNOT apply contract-based QoS, you cannot deny specific inter-EPG flows, and you cannot use Policy-Based Redirect (service insertion / firewall stitching) on that VRF \\u2014 capabilities a migration target design often depends on. Third, the flip from Unenforced to Enforced is the single most dangerous one-click change in ACI: the instant it is enforced, every flow that lacks a contract is dropped, so an undiscovered dependency becomes an immediate outage. Whether the migration\'s intent is to KEEP open communication or to IMPOSE segmentation, leaving the VRF silently Unenforced means the cutover\'s security and service-insertion behavior is undefined.", "observable": "The aci axis records each VRF contract enforcement preference (parse_aci_vrfs / moquery -c fvCtx).", "trigger": "snap[\'aci\'][host].vrfs has a VRF with pc_enf_pref=unenforced (default-permit between EPGs).", "recommended_action": "Make the policy explicit and contract-based BEFORE the cutover, and never flip the flag bare. (1) Decide the target posture for the VRF from the migration design (segmented, or intentionally open). (2) Profile the real traffic first \\u2014 use the fabric\'s flow/contract telemetry and connectivity/permit logs to enumerate which EPG pairs actually communicate, so you build contracts from evidence, not assumption. (3) If the intent is open-but-enforceable, switch the VRF to Enforced and reproduce the open behavior explicitly with a vzAny construct: have vzAny both provide and consume a single permit-any contract (functionally equivalent to Unenforced but now a real, enforced policy you can later tighten, and which unblocks QoS/deny/PBR). (4) Then progressively replace the permit-any with per-application contracts (or migrate selected EPGs into a Contract Preferred Group), tightening in stages while watching for drops. (5) Honor the constraint that vzAny-contracts and the Preferred Group are mutually exclusive in the same VRF \\u2014 pick one model. (6) Validate post-change that intended flows still pass and unintended ones are denied; if the decision is to remain Unenforced, record it as an explicit, signed-off risk exception so it never silently becomes \'healthy\'.", "alternatives": "The cheaper, riskier alternatives are either to leave the VRF Unenforced and migrate the flat posture as-is, or to flip it straight to Enforced in the maintenance window without first discovering and provisioning the required contracts.", "tradeoffs": "Leaving it Unenforced is free and zero-outage-risk at cutover but carries an unsegmented east-west blast radius and forfeits QoS/deny/PBR; the bare flip to Enforced achieves segmentation cheaply but, without prior flow discovery and contracts, drops every uncontracted flow the moment it takes effect \\u2014 turning a policy change into an outage.", "citation": "Cisco ACI Contract Guide White Paper (Enforced vs. Unenforced VRF semantics, any-any deny vs. any-any permit, Unenforced disables contract-based QoS/deny/PBR, Preferred Group); How To: vzAny Usage (vzAny provide+consume permit-any == Unenforced; vzAny vs. Preferred Group mutual exclusivity)", "engine_actionable": true}, {"id": "vxlan-evpn-control-plane-down", "title": "Restore the BGP L2VPN EVPN control plane before baselining or freezing the overlay fabric", "domain": "dc-overlay", "priority": "High", "design_intent": "In a VXLAN BGP-EVPN fabric, the BGP L2VPN EVPN address family is the control plane: VTEPs no longer flood-and-learn MACs in the data plane but instead advertise host reachability as EVPN routes \\u2014 Route Type 2 carries MAC and MAC/IP bindings for unicast forwarding, and Route Type 3 (Inclusive Multicast Ethernet Tag) advertises each VTEP\'s membership in a broadcast domain so BUM traffic can be replicated. When the EVPN session(s) are down, this distribution stops: remote VTEPs cannot install forwarding state for hosts behind the affected device, so overlay unicast between sites silently breaks and only locally-learned or flooded traffic survives. A down control plane is therefore not a cosmetic alarm \\u2014 it means the fabric is operating partially blind. For a brownfield migration this is a hard cutover gate: a baseline captured while EVPN is down records an incomplete MAC/IP table, masks which segments actually carry traffic, and risks a cutover that \'succeeds\' on the underlay while the overlay never converges. Often the session is administratively up at TCP level but the address family is not exchanging routes (route-target import/export or ASN-rewrite mismatch, missing inclusive-multicast routes), which looks healthy at a glance.", "observable": "The overlay axis records each BGP L2VPN EVPN neighbour state (parse_evpn_summary / show bgp l2vpn evpn summary).", "trigger": "snap[\'overlay\'][host].evpn_neighbors has a neighbour not Established (Idle/Active/Connect).", "recommended_action": "Work the control plane top-down only after the underlay is confirmed clean. (1) Verify the underlay/loopback reachability the BGP session rides on \\u2014 a flapping or unreachable spine/RR loopback presents as an EVPN session problem. (2) Run \'show bgp l2vpn evpn summary\' on the leaf and its route reflectors/spines; confirm the neighbor is Established and PfxRcd is non-zero. If the session is down, inspect the BGP neighbor state/last-error (AS number, update-source loopback, missing \'address-family l2vpn evpn\', RR client config, password/TTL). (3) If the session is up but no routes import, compare route-targets between sites and apply \'rewrite-evpn-rt-asn\' where ASN-auto RTs differ; verify Type-2 and Type-3 routes are present with \'show bgp l2vpn evpn\'. (4) Re-validate that L2RIB MAC routes are being redistributed into EVPN and that remote VTEPs now see them. For large-scale fabrics, extend BGP graceful-restart (e.g. toward 300s) so a high VNI-count reconvergence does not itself trigger a flap. Only re-snapshot the fabric once EVPN is Established and exchanging routes fabric-wide.", "alternatives": "The cheaper, riskier path is to treat one down EVPN adjacency as redundant (a second RR/spine still carries routes) and proceed to cutover, accepting degraded route diversity and slower convergence rather than pausing to root-cause; viable only if you positively confirm full Type-2/Type-3 coverage via the surviving path and the down session is genuinely a redundant peer, not the device\'s only route source.", "tradeoffs": "Fixing it first costs a maintenance-window detour and possibly an RR/ASN-policy change, but baselining or cutting over on a blind control plane risks an overlay-wide outage and a worthless pre-migration baseline \\u2014 the cost is trivial against the blast radius.", "citation": "IETF RFC 7432, BGP MPLS-Based Ethernet VPN (EVPN route types: Type-2 MAC/IP Advertisement, Type-3 Inclusive Multicast Ethernet Tag; control-plane MAC learning) \\u2014 corroborated by Cisco Nexus 9000 Series NX-OS VXLAN Configuration Guide, Configuring VXLAN BGP EVPN, and Cisco support doc \'Troubleshoot EVPN/VXLAN in Multi-Site Environment\' (show bgp l2vpn evpn summary; rewrite-evpn-rt-asn)", "engine_actionable": true}, {"id": "vxlan-nve-peer-down", "title": "Restore the down NVE (VTEP) peer before treating the affected segments as migratable", "domain": "dc-overlay", "priority": "High", "design_intent": "An NVE peer represents a remote VTEP with which this device has an active VXLAN tunnel; in NX-OS it appears in \'show nve peers\' with a State and a LearnType (CP for control-plane/EVPN, DP for data-plane). The VXLAN tunnel is a stateless IP-in-UDP encapsulation, so a peer only comes Up when (a) the underlay provides IP reachability between the two VTEP source loopbacks and (b) the two VTEPs share at least one VNI. A peer in Down state means the overlay path to everything behind that VTEP is broken even if the underlay otherwise looks fine and the EVPN routes still exist \\u2014 the device may hold the remote MAC/IP routes but cannot encapsulate to that destination. The usual root causes are underlay-specific: the remote VTEP\'s /32 loopback is not being advertised/reached (OSPF/IS-IS/BGP underlay adjacency, ECMP, or a black-holing fabric link), an NVE source-interface mismatch, or \\u2014 in a vPC pair \\u2014 the source-loopback (secondary/anycast VIP) coming up before the peer is truly ready. For a migration, a down VTEP peer means an entire leaf, vPC pair, or border\'s worth of endpoints is unreachable over the overlay; counting those segments as \'ready to move\' or capturing their state as the baseline would propagate an outage into the cutover.", "observable": "The overlay axis records each NVE (VTEP) peer state (parse_nve_peers / show nve peers).", "trigger": "snap[\'overlay\'][host].nve_peers has a peer in a non-Up (Down) state.", "recommended_action": "Diagnose bottom-up, since the NVE peer is a symptom of the underlay. (1) From \'show nve peers\', identify the peer IP (the remote VTEP loopback) and its State. (2) Confirm underlay reachability to that loopback: ping/traceroute the remote VTEP source-interface from this VTEP\'s source-interface, and verify the /32 is present via the underlay IGP/BGP (route present, correct next-hops, no flaps). (3) Verify both ends\' NVE config: \'show nve interface\' for the correct \'source-interface loopback\', host-reachability protocol bgp, and that the two VTEPs share at least one configured VNI. (4) For vPC VTEPs, confirm the NVE source-loopback hold-down timer is set below the vPC delay-restore timer so the VIP is not advertised before the overlay is ready, and check the anycast/secondary-IP configuration. (5) Once the loopback is reachable and config matches, the peer should transition to Up with LearnType CP; re-verify before clearing the segment for migration.", "alternatives": "The cheaper, riskier alternative is to bring the peer up by forcing data-plane discovery (e.g. sourcing host traffic / a ping across the overlay to populate the peer) and move on without fixing the underlay root cause \\u2014 it can flip the state Up transiently but leaves a fragile tunnel that will drop again under load or reconvergence, so it is only acceptable as a diagnostic confirmation, never as the fix.", "tradeoffs": "Properly restoring underlay reachability to the VTEP loopback may require touching IGP/ECMP or vPC timers in the window, but skipping it means the affected leaf/vPC\'s endpoints are dark over the overlay at cutover \\u2014 a localized fix versus a per-leaf outage.", "citation": "IETF RFC 8365, A Network Virtualization Overlay Solution Using EVPN (VTEPs as NVEs depend on IP reachability between NVEs; VXLAN UDP encapsulation; VNI as segment identifier) \\u2014 corroborated by Cisco support doc \'Configure VXLAN\' (118978) and Cisco Nexus 9000 NX-OS VXLAN Configuration Guide (show nve peers State/LearnType; source-interface loopback; vPC NVE source-loopback hold-down vs delay-restore)", "engine_actionable": true}, {"id": "vxlan-nve-vni-down", "title": "Restore the down NVE VNI (segment) before declaring that L2VNI/L3VNI ready for cutover", "domain": "dc-overlay", "priority": "High", "design_intent": "An NVE VNI is a single VXLAN segment instantiated on the VTEP \\u2014 an L2VNI bound to a VLAN/bridge-domain (Type BD), or an L3VNI bound to a tenant VRF for inter-subnet routing (Type VRF) \\u2014 shown in \'show nve vni\' with a per-VNI State of Up or Down. The VNI is the 24-bit identifier that maps a tenant broadcast domain (or VRF) onto the overlay; EVPN advertises that VNI in the MPLS-label field of its Type-2 routes and the VTEP\'s participation via Type-3. A VNI in Down state means that specific segment is not carried on this VTEP even when the box, its NVE peers, and the EVPN session are otherwise healthy \\u2014 so hosts in that VLAN/VRF on this leaf are isolated on the overlay. The dependencies are local and concrete: an L2VNI requires its VLAN to exist and be active with a correct VLAN-to-VNI mapping and a \'member vni\' under the NVE; an L3VNI requires the VRF, the VLAN/SVI used for the L3VNI (configured with \'ip forward\', not an IP address) to be up, and \'member vni <id> associate-vrf\' under the NVE. A down VLAN/bridge-domain or SVI drags its VNI down. For a migration this is the finest-grained cutover gate: a single down VNI silently strands one tenant subnet, so treating that segment as migrated \\u2014 or trusting a baseline that recorded it absent \\u2014 drops exactly that tenant\'s traffic at cutover while every fabric-level indicator stays green.", "observable": "The overlay axis records each NVE VNI state and type (parse_nve_vni / show nve vni).", "trigger": "snap[\'overlay\'][host].nve_vni has an L2VNI/L3VNI in a Down state.", "recommended_action": "Resolve from the local binding outward. (1) From \'show nve vni\' (or \'show nve vni <id> detail\'), confirm the VNI\'s State, Mode (CP), and Type (BD for L2VNI / VRF for L3VNI). (2) For an L2VNI Down: verify the VLAN exists and is active, the VLAN-to-VNI mapping is correct, and a matching \'member vni\' is present under the NVE interface; bring the VLAN/bridge-domain up. (3) For an L3VNI Down: verify the tenant VRF, that the L3VNI VLAN/SVI is up and uses \'ip forward\', and that \'member vni <id> associate-vrf\' is configured \\u2014 drive state by shut/no-shut on the VRF, never on the VNI interface. (4) Confirm the segment\'s evidence end-to-end: the VLAN/SVI is up, the VNI shows Up, and EVPN is advertising that VNI\'s Type-2/Type-3 routes to peers. (5) Only then mark the L2VNI/L3VNI ready and re-baseline that segment.", "alternatives": "The cheaper, riskier alternative is to exclude the single down VNI from the move group and cut over the rest of the leaf, deferring that one tenant segment to a follow-up window \\u2014 acceptable only if that tenant can tolerate staying on the legacy fabric temporarily and the exclusion is explicitly tracked, because the easy mistake is to assume the VNI will \'come up on its own\' post-cutover when its VLAN/SVI dependency is unmet.", "tradeoffs": "Fixing the VLAN/SVI/VRF binding is a small, well-scoped change, whereas shipping a down VNI strands one tenant\'s subnet at cutover behind an otherwise-green fabric \\u2014 a few minutes of remediation against a tenant-isolating, hard-to-spot outage.", "citation": "Cisco Nexus 9000 Series NX-OS VXLAN Configuration Guide, Configure VXLAN / Configure VXLAN BGP EVPN (show nve vni State Up/Down, Type BD vs VRF; VLAN-to-VNI mapping and member vni; L3VNI SVI \'ip forward\' and member vni associate-vrf; drive L3VNI via VRF shut/no-shut) \\u2014 RFC 8365 corroborates VNI-to-broadcast-domain mapping and EVPN carriage of the VNI", "engine_actionable": true}, {"id": "mpls-ldp-session-down", "title": "Restore the down LDP session before baselining or cutting over the MPLS underlay", "domain": "sp-mpls", "priority": "High", "design_intent": "LDP is the control protocol that hands out the transport labels for every IGP /32 loopback in the core, so a down LDP session means a stretch of the label-switched underlay has gone dark: no Label Mapping is being exchanged across that adjacency, and any bindings that existed have been withdrawn (LABEL_WITHDRAW / Label Release), collapsing the LSPs that traversed it. LDP comes up in two stages \\u2014 UDP Hello discovery, then a TCP session policed by a KeepAlive/holdtime timer that declares the peer dead the moment keepalives are missed (RFC 5036) \\u2014 so a \'session down\' is unambiguous protocol state, not a soft warning. Because LDP only labels routes the IGP already advertises, the session almost always drops for a reason a layer below it: the IGP adjacency or the transport/router-ID loopback went unreachable, an ACL or MTU change broke the TCP/646 path, or label-protocol config drifted. Operationally this is the foundation every L3VPN and L2VPN service rides on, so until it is healthy every higher-layer \'up\' is suspect. For a brownfield cutover it is a hard gate: a baseline captured with an LDP session down records a phantom underlay, post-cutover diffs cannot distinguish \'we broke it\' from \'it was already broken,\' and the classic LDP-vs-IGP race (IGP forwards over a link before LDP has labeled it) silently blackholes labeled traffic the instant a link flaps in the window.", "observable": "The mpls axis records each LDP neighbour session state (parse_mpls_ldp_neighbors / show mpls ldp neighbor).", "trigger": "snap[\'mpls\'][host].ldp_neighbors has a neighbour not operational (Down).", "recommended_action": "Work bottom-up. (1) Confirm the IGP underneath the session is up and the peer\'s transport address / router-ID loopback is in the RIB \\u2014 `show mpls ldp neighbor` / `show mpls ldp discovery` (IOS XR: `show mpls ldp neighbor brief`) to see whether you are stuck at Hello/discovery or at session establishment. (2) If discovery is fine but the TCP session won\'t form, verify L3 reachability to the LDP transport address and that nothing blocks TCP/646 or has lowered path MTU; check `mpls ldp router-id`/transport-address pinning and that label protocol is enabled on the interface. (3) Inspect label exchange with `show mpls ldp bindings` / `show mpls forwarding-table` to confirm bindings return. (4) Bounce only the affected adjacency with `clear mpls ldp neighbor <ip>` (never restart the LDP process under traffic). (5) Harden against recurrence before cutover: enable LDP-IGP synchronization so the IGP advertises max-metric on a link until LDP is converged (prevents the blackhole), and LDP session protection so a backup targeted-hello adjacency holds the session and its label bindings up across a direct-link flap.", "alternatives": "The cheaper path is to bring the link/session back, see the LSP repopulate, and proceed without enabling LDP-IGP sync or session protection \\u2014 accepting the IGP/LDP race and a full reconverge on the next flap. The riskier shortcut is to cut over on the assumption that traffic \'is flowing\' because the IGP path is up, even though that path is unlabeled.", "tradeoffs": "Configuring LDP-IGP sync and session protection costs a few minutes of change and a small amount of targeted-hello state, but it converts a silent labeled-traffic blackhole during link flaps into a graceful, lossless reconvergence \\u2014 cheap insurance on a path that carries every VPN.", "citation": "RFC 5036 (LDP Specification) \\u2014 sessions, KeepAlive/holdtime, label distribution; and Cisco MPLS Configuration Guide \\u2014 MPLS LDP IGP Synchronization (blackhole prevention)", "engine_actionable": true}, {"id": "mpls-l3vpn-vpnv4-down", "title": "Triage the L3VPN underlay before the VPNv4 overlay, then prove the two-label path before cutover", "domain": "sp-mpls", "priority": "High", "design_intent": "An L3VPN works only when two independent planes agree: an MP-BGP VPNv4 control plane that carries each customer prefix tagged with a Route Distinguisher (so overlapping address space stays distinct) and Route-Target extended communities that drive VRF import/export, and an MPLS data plane that stacks two labels \\u2014 an outer transport label that delivers the packet to the egress PE\'s BGP next-hop loopback over an LSP, and an inner VPN label that the egress PE assigned to select the right VRF and CE (RFC 4364). A \'VPNv4 down\' decision therefore covers two very different faults with the same symptom of lost PE-PE reachability: the MP-BGP session itself is down (or the address family isn\'t activated, or extended-community/next-hop policy is wrong, or RT import/export no longer matches), or the session is fine and routes are present but the outer LSP to the remote PE is broken so the labeled packet has nowhere to go. The asymmetry matters: if BGP drops, VRFs keep forwarding on already-installed routes until they age out, masking the fault briefly; if the LSP drops, forwarding fails immediately even though `show ip route vrf` still looks perfect. For a brownfield migration this is the highest-stakes gate \\u2014 it is live customer traffic with an SLA \\u2014 and \'the route is in the table\' is not proof of forwarding; only an intact two-label path is.", "observable": "The mpls axis records each VPNv4 BGP neighbour state (parse_bgp_vpnv4_summary / show bgp vpnv4 unicast summary).", "trigger": "snap[\'mpls\'][host].vpnv4_neighbors has a VPNv4 session not Established.", "recommended_action": "Follow Cisco\'s top-down-then-underlay sequence. (1) Establish the control plane: `show bgp vpnv4 unicast all summary` (IOS XR: `show bgp vpnv4 unicast summary`) \\u2014 is the PE-PE/RR session Established and is the VPNv4/MP-BGP address family activated with send-community extended? (2) Confirm policy: the prefix is present with the right RD and its RTs match the remote VRF\'s import list \\u2014 `show bgp vpnv4 unicast all <prefix>` and `show vrf <name>` for import/export RTs; a one-sided RT edit is the classic silent break. (3) Confirm the route landed in the VRF and carries a VPN label \\u2014 `show ip route vrf <name>` and `show bgp vpnv4 unicast all tags` / `show ip cef vrf <name> <prefix> detail`. (4) Now prove the UNDERLAY the overlay depends on: is the remote PE\'s BGP-next-hop /32 reachable AND label-switched end-to-end? `show mpls forwarding-table <nexthop-/32>` on the ingress PE and at each P hop, then validate the LSP with `ping mpls ipv4 <nexthop>/32` (LSP ping). The order is deliberate: a VPNv4 symptom is frequently an LDP/IGP fault one layer down (this is why the LDP-session-down principle is a prerequisite). (5) Remediate the layer that actually failed \\u2014 re-peer/activate AF, re-align RTs, or fix the transport LSP \\u2014 and only then re-baseline.", "alternatives": "The cheaper alternative is to confirm the VPNv4 prefixes are back in the VRF and accept that as \'fixed\' without LSP-pinging the next-hop, trusting that the underlay is healthy. The riskier shortcut is to bounce the BGP session or flap the VRF hoping it re-converges, which can churn every customer prefix on that PE and turn a single-VRF incident into a fleet event.", "tradeoffs": "Verifying the actual two-label forwarding path with an LSP ping costs one extra command per affected next-hop, but it is the only thing that distinguishes a control-plane-clean VRF from one that is silently blackholing \\u2014 the difference between a real go and a cutover that fails the moment the maintenance window closes.", "citation": "RFC 4364 (BGP/MPLS IP VPNs) \\u2014 VPNv4 routes, RD/RT, two-level label stack, PE-reachability requirement; and Cisco \'How to Troubleshoot the MPLS VPN\' (Doc ID 13734) for the show-command sequence", "engine_actionable": true}, {"id": "mpls-l2vpn-pseudowire-down", "title": "Separate the attachment-circuit fault from the pseudowire/underlay fault before declaring the L2 circuit cut", "domain": "sp-mpls", "priority": "High", "design_intent": "An AToM/VPWS pseudowire emulates a point-to-point L2 circuit by binding the two PEs with a targeted LDP session that exchanges a VC (PW) label in each direction; that label is the demultiplexor the egress PE uses to map a received frame back to the right circuit, and the whole exchange rides inside an MPLS PSN tunnel (the transport LSP) that must already exist between the PEs (RFC 4447). For the PW to come up, both ends must agree on the PWid/VC-ID and PW type \\u2014 a non-zero connection ID that, with the PW type, names the circuit and MUST match in both directions \\u2014 and on parameters such as MTU; a mismatch leaves the PW down even though signaling reached the peer. A \'pseudowire down\' decision can mean any of three distinct things, and conflating them is the trap: the local attachment circuit (the customer-facing sub-interface/EFP) is down, so the PW is signaled down via the PW Status TLV; the two PEs disagree on VC-ID/MTU/encap so no consistent label binding forms; or the AC is fine and config matches but the targeted LDP session / transport LSP between PEs is broken so the VC label can\'t be exchanged or the labeled frame can\'t be carried. `show mpls l2transport vc detail` / `show l2vpn xconnect detail` exposes exactly which: local vs remote VC label populated, AC state, and the signaled status. For a brownfield cutover this gates a real customer L2 service, and because a PW depends on the same LDP/LSP underlay as everything else, a PW-down is often the first visible symptom of an underlay problem \\u2014 so it must be diagnosed precisely, not just retried.", "observable": "The mpls axis records each L2VPN pseudowire status (parse_mpls_l2vpn_vc / show mpls l2transport vc).", "trigger": "snap[\'mpls\'][host].l2vpn_vcs has a VC/pseudowire not UP.", "recommended_action": "(1) Read the state first: `show mpls l2transport vc <id> detail` (IOS XR: `show l2vpn xconnect detail`) and split the symptom \\u2014 is the local AC down, is the remote VC label missing (peer/underlay problem), or are both labels present but the PW still down (parameter mismatch)? (2) Fix the attachment circuit if the AC is down \\u2014 the customer-facing interface/EFP, its encapsulation, and link/line-protocol state \\u2014 because no PW comes up over a dead AC. (3) Reconcile the signaling parameters end-to-end: the VC-ID/PW-ID and PW type must match on both PEs, and MTU/encapsulation must agree (note IOS XR counts the L2 header in its MTU, a frequent IOS-XE\\u2194IOS-XR mismatch); confirm `encapsulation mpls`/pw-class is present. (4) If the AC is up and parameters match but the remote VC label is absent, drop to the underlay: confirm the targeted LDP session to the peer is up and the transport LSP to the peer loopback is label-switched \\u2014 `show mpls ldp neighbor`, `show mpls forwarding-table <peer-/32>`, and validate with a VCCV/PW ping (`ping mpls pseudowire \\u2026`). (5) Remediate the layer that failed and re-verify the PW shows UP/UP (AC up, PW up) before re-baselining.", "alternatives": "The cheaper alternative is to bounce the xconnect/AC and watch it re-signal up without confirming the VCCV path, accepting that the data plane is healthy because the control plane went UP. The riskier shortcut is to disable PW status signaling (`no status`) to force the circuit \'up\' when a peer seems not to honor it \\u2014 which can hide a genuine AC fault behind a green light.", "tradeoffs": "Walking AC \\u2192 parameters \\u2192 underlay and finishing with a VCCV/PW ping costs a few extra minutes versus a blind bounce, but it stops you certifying a circuit that signals UP while silently dropping customer frames, and it correctly attributes a PW-down to its true cause instead of masking an underlay fault.", "citation": "RFC 4447 (Pseudowire Setup and Maintenance Using LDP) \\u2014 targeted LDP, PWid FEC/VC-ID match, VC label, PW Status TLV, PSN-tunnel dependency; and Cisco \'Understand MPLS L2VPN Pseudowire\' (Doc ID 213238)", "engine_actionable": true}, {"id": "sdwan-control-connection-down", "title": "Restore the down WAN-edge control connection before freezing the SD-WAN cutover baseline", "domain": "sd-wan", "priority": "High", "design_intent": "A Catalyst SD-WAN WAN edge builds permanent DTLS (default, UDP 12346+) or TLS control connections to the orchestration plane: first to vBond (authentication, authorization and NAT discovery), then to vManage (management/NETCONF) and vSmart (which carries OMP). These tunnels are mutually authenticated by certificate, so a connection that the engine reports as down almost always resolves to one of a small, deterministic set of root causes the controller history names explicitly: DCONFAIL (DTLS handshake/packet loss), CRTVERFL (peer certificate could not be verified \\u2014 usually a missing enterprise root-CA on the edge), CTORGNMMIS (organization-name mismatch), SERNTPRES / BIDNTVRFD (the chassis UUID + certificate serial is not an authorized entry in the WAN-edge list), or a clock skew that makes a valid certificate look expired. Critically, the data plane is independent of this control plane \\u2014 IPsec tunnels with BFD keep forwarding, and OMP graceful restart caches routes for 12 hours by default \\u2014 so a down control connection produces NO immediate user-visible outage. That is exactly why it is a brownfield trap: the site looks healthy at the data plane while it is actually a single controller event (or a re-key) away from going dark, and any policy/route change pushed during the migration will silently never reach that edge. Baselining a fabric with a partitioned control connection bakes a latent blackhole into the cutover.", "observable": "The sdwan axis records each WAN-edge control connection state / actual-vs-expected count (parse_sdwan_control_connections / vManage /dataservice/device/control/connections).", "trigger": "snap[\'sdwan\'][host].control_connections has a connection state=down or actual<expected.", "recommended_action": "Work the controller\'s own verdict, outside-in: (1) read `show sdwan control connections-history` and pivot on the LOCAL/REMOTE ERROR mnemonic rather than guessing. (2) Prove underlay reachability and routing to each controller TLOC (ICMP/traceroute, valid RIB next-hop, correct color/public-private mapping, and firewall/NAT pinholes for 12346+/DTLS or the TLS port). (3) Verify time: confirm NTP is synchronized on the edge and controllers (a skewed clock invalidates certificates and breaks the handshake). (4) Validate identity: certificate validity and the enterprise root-CA installed on the edge (`show sdwan certificate root-ca-cert`), the organization-name match, and that the device UUID/serial is authorized \\u2014 then re-push the WAN-edge list with Configuration > Certificates > Send to Controllers. (5) Re-check that all expected connections (vBond/vManage/vSmart, both transports) are UP and stable, not flapping. Resolve to green and re-snapshot before the cutover gate.", "alternatives": "The cheaper move is to accept a partial control plane \\u2014 e.g. one of two transports/controllers up \\u2014 and proceed, leaning on the fact that the data plane is still forwarding and graceful restart will hold routes for hours. It defers the fix without a maintenance window.", "tradeoffs": "Cutting over on a partial or down control connection trades a few hours of certificate/NTP/reachability triage now for an invisible single-point-of-failure that surfaces as an unreachable site the moment a controller blips, a certificate re-keys, or the 12-hour graceful-restart cache expires.", "citation": "Cisco \\u2014 Troubleshoot SD-WAN Control Connections (Doc ID 214509; control connections-history error codes DCONFAIL/CRTVERFL/CTORGNMMIS/SERNTPRES/BIDNTVRFD, DTLS/TLS, certificate + root-CA + WAN-edge-list checks)", "engine_actionable": true}, {"id": "sdwan-omp-peer-down", "title": "Recover the down OMP peering to vSmart before its 12-hour graceful-restart cache hides the loss", "domain": "sd-wan", "priority": "High", "design_intent": "OMP is the BGP-like control protocol that runs INSIDE the DTLS/TLS control connection to vSmart; it peers on the two devices\' system IPs (exactly one OMP session per edge\\u2194vSmart, independent of how many TLOCs/transports exist) and is how the edge learns the overlay: OMP/vRoutes, TLOC routes, service routes, encryption keys and centralized policy. When the engine reports an OMP peer down, the edge is no longer exchanging overlay reachability with that controller. Because OMP graceful restart is enabled by default, the consequence is deferred, not immediate: the previously learned routes go from ACTIVE to STALE and the edge keeps forwarding on the cached information for the graceful-restart timer \\u2014 43,200 seconds (12 hours) by default, range 1s\\u20137 days. If the session never recovers, those stale routes are flushed at timer expiry and the site loses its overlay paths; on recovery, an end-of-RIB marker fires ~300s after the peer comes up and anything not refreshed is deleted. This is the most dangerous of the three to baseline through, because it sits at the routing layer: as long as one vSmart in the cluster is still peered the overlay may look fine, but the redundancy is gone and any new prefix, policy, or move-group route advertised during the migration will not propagate through the dead peer \\u2014 producing partial reachability or a hard blackhole at cutover, not a clean failure.", "observable": "The sdwan axis records each edge OMP peer-down counter (parse_sdwan_omp_counters / vManage /dataservice/device/counters).", "trigger": "snap[\'sdwan\'][host].omp_counters has omp_down>0 on an edge.", "recommended_action": "(1) Confirm the OMP state and scope with `show sdwan omp peers` \\u2014 read the peer state (Up vs init-in-gr) and the R/I/S (received/installed/sent) counters; a peer stuck in graceful restart with stale routes is living on borrowed time. (2) Since OMP rides the control connection, first rule out a control-connection fault to that vSmart (history error code, reachability, certificate, NTP) \\u2014 fix that and OMP typically re-establishes. (3) If the control connection is up but OMP is not converging, check OMP-layer health: hold-time/timer mismatch (Cisco recommends 300s hold time), graceful-restart configuration, and `send-path-limit` / route-count limits that can starve advertisement. (4) Use OMP vDiagnostics (auto-captures a snapshot on peering loss) to root-cause hello/timer/flap behavior. (5) Verify both vSmarts in the cluster are peered (redundancy restored) and that received/installed counts match the expected prefix set before re-baselining.", "alternatives": "The cheaper path is to rely on the surviving vSmart peer plus the 12-hour graceful-restart cache and proceed \\u2014 accept reduced control-plane redundancy temporarily rather than schedule the fix.", "tradeoffs": "You trade an OMP/control-connection investigation now for a fabric running non-redundant on a stale-route cache: a second vSmart hiccup, or simply the 12-hour timer expiring, flushes the cached overlay and blackholes the site \\u2014 and any route/policy you pushed during cutover quietly never reached the edges behind the dead peer.", "citation": "Cisco \\u2014 Routing Configuration Guide, Catalyst SD-WAN Releases 17.x: OMP and OMP graceful restart (OMP over DTLS/TLS, ACTIVE/STALE route states, default GR timer 43,200s/12h, EOR marker; recommended 300s hold time)", "engine_actionable": true}, {"id": "sdwan-device-unreachable", "title": "Re-establish the unreachable device\'s controller reporting before treating its state as known-good", "domain": "sd-wan", "priority": "High", "design_intent": "A device the engine flags as unreachable / not reporting is one vManage (and the wider control plane) can no longer see \\u2014 typically signalled by a peer-timeout in the connection history: VB_TMO, VM_TMO, VP_TMO, VS_TMO mark loss of reachability to the vBond, vManage, peer-edge and vSmart respectively. The decisive nuance for a migration is that \'unreachable to the controller\' is a CONTROL/MANAGEMENT-plane statement, not necessarily a forwarding statement: because the data plane (IPsec + BFD) is independent of the DTLS/TLS control plane and OMP graceful restart caches routes, a device can still be passing user traffic while it is invisible to vManage. That invisibility is corrosive to a brownfield assessment in three ways. First, coverage honesty: an unreachable device contributes NO fresh evidence, so its inventory, health, certificate and software state are UNKNOWN \\u2014 they must never be silently scored as healthy. Second, it cannot receive configuration, templates, certificate updates, or software during the migration \\u2014 it will be skipped by every push. Third, the unreachability itself usually traces to a real fault (underlay/routing loss, an expired or unsynced certificate from clock skew, a stale WAN-edge-list entry, or vManage-side scale/NMS load) that will block or corrupt the cutover for that asset. Freezing a baseline that includes an unreachable, unverifiable device is the textbook way a migration discovers a stranded site only after the window closes.", "observable": "The sdwan axis records each device controller-reported reachability (parse_sdwan_devices / vManage /dataservice/device).", "trigger": "snap[\'sdwan\'][host].devices has a device reachability=unreachable.", "recommended_action": "(1) Read the connection history and identify which peer timed out (VB_TMO/VM_TMO/VP_TMO/VS_TMO) \\u2014 that names the lost relationship. (2) Establish underlay reachability to the controllers (ICMP/traceroute) and confirm a valid RIB route with the correct next-hop/TLOC; control connections never form over a broken underlay. (3) Check time first \\u2014 verify NTP/clock sync on the device and controllers, since skew makes a good certificate look expired and silently breaks the handshake. (4) Validate identity: certificate validity, enterprise root-CA present on the device, organization-name match, and the device serial/UUID in the WAN-edge list \\u2014 then Configuration > Certificates > WAN Edge List > Send to Controllers to re-sync. (5) If reachability/identity are clean but the device is still not reporting, look vManage-side (NMS/DPI load, cluster/disk health). (6) Confirm the device is back to a full, stable control state and re-collect its evidence so the baseline reflects observed \\u2014 not assumed \\u2014 state.", "alternatives": "The cheaper alternative is to carry the device forward on its last-known (stale) inventory and proceed, assuming the data plane is fine because graceful restart is still holding \\u2014 deferring the reachability/certificate fix instead of taking a window to restore reporting.", "tradeoffs": "You trade a reachability/NTP/certificate restoration now for a device that is unmanageable and unverifiable during the cutover: it silently misses every config/software push, its true state stays UNKNOWN, and if its data plane was only surviving on cached OMP state you discover a stranded or mis-built site after the maintenance window has closed.", "citation": "Cisco \\u2014 Troubleshoot Connection When Devices Lose Connectivity to vManage (Doc ID 218077; peer-timeout codes VB_TMO/VM_TMO/VP_TMO/VS_TMO, reachability + RIB/TLOC, certificate \'Send to Controllers\', NTP/clock sync)", "engine_actionable": true}, {"id": "fhrp-resilience-tracking-and-preempt", "title": "Make gateway failover deterministic: pair FHRP preempt with interface/object tracking before baselining", "domain": "routing", "priority": "High", "design_intent": "A first-hop redundancy group (HSRP/VRRP/GLBP) only protects the default gateway against the failure modes it is told to watch. The engine found groups that exist but carry no interface or object tracking and/or no preempt, which means failover is not deterministic. Tracking ties the group\'s priority to the health of the device\'s own uplink or a tracked object (interface line-protocol, IP-routing state, an IP SLA/route): when that uplink fails, IOS decrements the group priority (default 10) so a healthier peer can take the gateway. But that decrement only causes an actual hand-off if preempt is enabled on the peer that should take over: per RFC 2281 a higher-priority router takes the active role via a Coup only when preemption is configured, otherwise it stays Standby and the role never moves. The dangerous case is a router that is Active for the virtual IP/MAC but has lost its own northbound path: without tracking it keeps advertising itself as the gateway and black-holes every host that ARPs for it, because hosts see the FHRP hellos as healthy. Equally, without preempt the intended primary will not reclaim the gateway after recovery, so the fleet silently runs off the wrong (often lower-capacity or asymmetric) router until the next failure. Both leave the post-cutover gateway behavior non-deterministic \\u2014 exactly the property a migration baseline must guarantee.", "observable": "The fhrp_detail axis records per active gateway its interface/object tracking and preempt config (parse_hsrp_detail / show standby).", "trigger": "snap[\'fhrp_detail\'][host] has an Active gateway with no tracking or no preempt.", "recommended_action": "Resolve per group, primary side first: (1) Decide intended roles \\u2014 set explicit priorities (do not leave both peers at the 100 default) so primary > secondary, and document which device should own each VLAN\'s gateway. (2) Add tracking on the device whose uplink loss should trigger hand-off \\u2014 prefer enhanced object tracking (track an object for line-protocol or IP-routing state, or an IP SLA/route reachability object) over the legacy \'standby track <if>\' so a tracked-uplink failure decrements priority below the peer; size the decrement so the crossover actually occurs (decrement > priority gap). (3) Enable preempt on the higher-priority router so it both reclaims after recovery and so a tracking-driven decrement on its peer results in a real failover. (4) Add \'preempt delay minimum\' (and \'standby delay minimum reload\') so a recovering/rebooting router waits for its routing table and uplinks to converge before seizing the gateway, avoiding a transient black hole. (5) For dual-supervisor/stacked platforms keep SSO/HSRP-aware redundancy so a switchover doesn\'t bounce the role. (6) Re-collect and confirm tracking decrements and preempt are present and roles match design before freezing the baseline.", "alternatives": "The cheaper path is to enable preempt alone (so the primary at least reclaims the gateway after recovery) and skip tracking. This restores predictable role ownership but leaves the worst failure unguarded: an Active router that loses its own uplink still advertises itself as the gateway and black-holes the subnet, because nothing decrements its priority. A second shortcut \\u2014 raising priority but never enabling preempt on the primary \\u2014 gives the appearance of a designed hierarchy while guaranteeing the primary never actually takes the role back.", "tradeoffs": "Adding tracking + tuned preempt/preempt-delay costs a short design and change-window effort per group and a brief, controlled failover during testing, in exchange for converting an undefined, evidence-blind gateway behavior into deterministic sub-second-to-seconds failover that the cutover acceptance test can actually prove.", "citation": "Cisco IOS XE First Hop Redundancy Protocols Configuration Guide \\u2014 Configuring HSRP (standby preempt, standby track / object tracking, preempt delay); Cisco support doc \'Use Standby Preempt and Standby Track Commands\'; RFC 2281 Cisco Hot Standby Router Protocol (priority/preemption/Coup, 3s/10s timers)", "engine_actionable": true}, {"id": "reconcile-broken-fhrp-before-cutover", "title": "Reconcile any broken FHRP group (no-active, dual-active/split, or split-brain) before freezing the cutover baseline", "domain": "routing", "priority": "High", "design_intent": "A healthy first-hop redundancy group has exactly one Active router forwarding for the virtual IP/MAC and one Standby tracking it (RFC 2281: \'Only the active router forwards the packets sent to the virtual router\'). The engine fired because a group is in a state that violates this invariant \\u2014 no router is Active (the gateway is dark and hosts have no first hop), both peers are Active (dual-active / split-brain, so two devices answer for the same MAC), or the peers cannot see each other (split \\u2014 each believes it is the sole gateway). The most common physical cause is loss of the L2 path that carries the FHRP hellos between the peers: when the inter-switch link/port-channel carrying the group VLAN goes down, each side stops hearing the other\'s hellos within the holdtime (default 10s) and promotes itself, so the subnet is split into two active gateways and traffic crossing the partition is black-holed. Other documented causes are a unidirectional link, an ACL or control-plane policing filtering the HSRP transport (multicast 224.0.0.2 / UDP 1985, FF02::66 for HSRPv2 IPv6), high CPU dropping hellos, an MTU or authentication mismatch between peers, or a timer/version (HSRPv1 vs v2) mismatch. Whatever the trigger, the gateway\'s behavior is currently undefined and partition-dependent. Cutover validation hinges on a deterministic gateway: if the baseline is captured while the group is split or dual-active, the pre/post diff is measured against a broken reference, the \'redundancy verified\' acceptance gate is false, and the very next link event during the migration window can drop production traffic.", "observable": "The fhrp axis records per-VLAN FHRP consistency issues (compute_fhrp_consistency: split-brain / mixed protocol / mismatched group|VIP).", "trigger": "snap[\'fhrp\'] has a group with a broken-but-present state (issues populated).", "recommended_action": "Treat this as a fault to clear before any cutover-readiness sign-off, diagnosing toward the single invariant \'one Active, one Standby\': (1) On both peers capture FHRP state and history (e.g. show standby / show standby brief, show standby <vlan>, plus the group\'s recent state transitions and any STANDBY-3-DUPADDR or \'received own hello\' logs) to classify the state and find the last transition\'s trigger. (2) Verify the L2 path carrying the group VLAN between the peers is up end-to-end \\u2014 the trunk/port-channel, the VLAN\'s allowed list, and STP forwarding for that VLAN; a down or pruned inter-switch link is the first suspect for a split. (3) Confirm the peers can actually exchange hellos: check that the HSRP transport (multicast 224.0.0.2 / UDP 1985, or IPv6 FF02::66) is not filtered by an ACL or dropped by CoPP, that the link is bidirectional, and that MTU is consistent. (4) Reconcile configuration symmetry \\u2014 same group number, same HSRP version, matching authentication, and consistent timers on both peers. (5) Rule out resource exhaustion \\u2014 CPU/interface drops starving the hellos. (6) Once a single Active is restored, set explicit priority + preempt + tracking so the recovered state is also deterministic, then re-collect and confirm one-Active/one-Standby before baselining.", "alternatives": "The cheaper, riskier alternative is to accept the degraded group and cut over anyway, planning to fix it in the next window \\u2014 defensible only when the affected VLAN(s) carry no production gateway traffic and that is evidenced, not assumed. A second shortcut is to collapse the group to a single gateway (remove the broken peer from the equation) so at least one Active is unambiguous; this stops the split immediately but knowingly ships the cutover with zero first-hop redundancy on that subnet, turning the gateway into a single point of failure.", "tradeoffs": "Reconciling first costs investigation time and possibly a maintenance touch on the inter-switch link or an ACL/CoPP/timer change before the gate clears, but it is the difference between validating the cutover against a deterministic single-gateway baseline and certifying redundancy on top of an active black-hole that the next link flap will expose.", "citation": "Cisco support doc \'Troubleshoot HSRP Common Issues\' (dual-active / duplicate-address / lost-hello causes and fixes) and \'Avoiding HSRP Instability in a Switching Environment\'; RFC 2281 Cisco Hot Standby Router Protocol (single active router forwards; hello/hold 3s/10s; active-timer expiry \\u2192 standby becomes active)", "engine_actionable": true}, {"id": "ipv6-duplicate-address-dad-failure", "title": "Resolve the duplicate IPv6 address (and free the DUPLICATE-state interface) before baselining the fabric", "domain": "ipv6", "priority": "High", "design_intent": "Duplicate Address Detection (DAD) is the Neighbor Discovery probe a node runs before it commits any unicast IPv6 address to an interface: it multicasts a Neighbor Solicitation for the tentative address and waits, and if any other node answers, the address is a proven duplicate on that link. Per RFC 4862, a duplicated address MUST NOT be assigned, so the engine\'s fired condition means the interface is carrying a configured address that the stack has refused to use \\u2014 it sits in DUPLICATE/dadfailed state and silently discards traffic to it. The severity hinges on which address collided. If the duplicate is the interface link-local (fe80::) address, RFC 4862 says IPv6 operation SHOULD be disabled and on Cisco IOS-XE/IOS-XR the platform disables IPv6 processing on the whole interface \\u2014 so the link is not partially degraded, it is dark for IPv6, and every protocol that rides the link-local (OSPFv3, EIGRP for IPv6, ND, even the next-hop of installed routes) goes with it. In a brownfield migration this is a cutover landmine: a static address typo, a cloned/templated config pushed to two devices, a re-used virtual/HSRPv2 or anycast-gateway address, or two SVIs bridged onto the same VLAN will pass a casual \'show running\' review yet leave a gateway or transit hop unreachable the instant the new path is cut in. It must be cleared before the baseline is frozen, because a snapshot taken with an address in DUPLICATE state records a reachability state that does not actually exist.", "observable": "The ipv6_nd axis records per-interface IPv6 address DAD state (parse_ipv6_interface_addrs / show ipv6 interface).", "trigger": "snap[\'ipv6_nd\'][host].interfaces has a global/link-local address in DAD DUPLICATE.", "recommended_action": "1) Identify the offending address and scope with \'show ipv6 interface <intf>\' (look for DUPLICATE/dadfailed and whether it is the link-local or a global/anycast address). 2) Find the true owner of the address on the link \\u2014 check the neighbor table / ND cache and the L2 path (MAC behind the solicited-node multicast); on a switched segment trace the duplicate to a second device or a mis-bridged VLAN rather than assuming a stack glitch. 3) Fix the root cause, not the symptom: correct the typo\'d static address, remove the duplicate from whichever device is the imposter, or (for an EUI-64/link-local collision from a cloned MAC or VM) re-base the interface identifier. 4) Only after the conflict is genuinely gone, return the address to service \\u2014 on IOS-XR/IOS-XE clear the latched state with \'clear ipv6 duplicate-address <interface>\' (or shut/no-shut, which re-runs DAD), since the DUPLICATE flag does not self-heal once set. 5) Re-verify with \'show ipv6 interface\' that the address is now VALID/up and confirm the dependent routing adjacency re-forms. Tune \'ipv6 nd dad attempts\' only as a deliberate design choice (e.g. set to 0 on a known-clean point-to-point link), never as a way to mask a real collision.", "alternatives": "The cheaper, riskier path is to clear the DUPLICATE state (or disable DAD with \'ipv6 nd dad attempts 0\') to make the address usable again without first proving the conflict is gone, or to simply renumber the local interface to dodge the clash. This restores the interface in seconds, but if the real duplicate is still live you now have two nodes answering for one address \\u2014 intermittent black-holing, ND cache thrash, and a defect that moves with whichever host last spoke.", "tradeoffs": "Doing it right costs an investigation cycle and possibly a second device\'s change window, but it removes the actual address conflict so the cutover reachability is real; suppressing DAD trades minutes saved now for a non-deterministic, hard-to-localize outage during or after the migration.", "citation": "IETF RFC 4862, IPv6 Stateless Address Autoconfiguration, \\u00a75.4 Duplicate Address Detection (esp. 5.4.4 \'When Duplicate Address Detection Fails\' / 5.4.5), and Cisco IOS-XR IP Addresses and Services Command Reference, \'clear ipv6 duplicate-address\' / DUPLICATE-state behavior", "engine_actionable": true}, {"id": "ipv6-routing-adjacency-down", "title": "Restore the down IPv6 routing adjacency before freezing the routing baseline", "domain": "ipv6", "priority": "High", "design_intent": "A routing adjacency is the contract by which two devices agree to exchange reachability: OSPFv3 and EIGRP for IPv6 build a per-link neighbor relationship and flood LSAs/updates over it, while BGP rides a TCP/179 session and advertises prefixes only once that session reaches Established. The engine\'s fired condition means one of these relationships is down, so the prefixes that should be learned across it are absent and the device is forwarding on a stale or incomplete table \\u2014 a per-protocol, directional blind spot rather than a whole-box outage. The cause is almost always a one-sided mismatch rather than a hard link failure: for OSPFv3 it is mismatched hello/dead timers, area ID, area type (stub/NSSA), authentication, MTU (the classic stuck-in-EXSTART/Exchange symptom), or an ACL dropping IP protocol 89 \\u2014 and critically, because OSPFv3 sources its packets from the interface link-local address, a link-local that is missing or sitting in DAD-DUPLICATE state will hold the adjacency down even though the global addresses look fine. For BGP it is no IP route to the peer address, TCP/179 blocked by an ACL or control-plane filter, a wrong remote-AS, an MD5/TCP-AO key mismatch, a missing \'update-source\' on a loopback peering, a missing \'ebgp-multihop\' to a non-directly-connected eBGP peer, or hold-timer expiry from packet loss/MTU. In a brownfield migration a down adjacency is a hard cutover gate: it frequently exposes the exact asymmetry the new design introduces \\u2014 a renumbered link, a new firewall/ACL in the path, an MTU step on a new transit, or a router-ID/AS collision \\u2014 and if you cut over while it is down, the traffic that was meant to follow the freshly-learned prefixes black-holes. A baseline captured with the adjacency down also misrepresents which routes the device actually has.", "observable": "The ipv6_routing axis records OSPFv3 / BGP-IPv6 neighbour states (parse_ospfv3_neighbors / parse_bgp_ipv6_summary).", "trigger": "snap[\'ipv6_routing\'][host] has an OSPFv3/BGP-IPv6 adjacency not full/established.", "recommended_action": "1) Read the exact stuck state first \\u2014 \'show ospfv3 neighbor\' / \'show ipv6 ospf neighbor\' or \'show bgp ipv6 unicast summary\' \\u2014 because the state names the failing phase (INIT/EXSTART vs Idle/Active) and points straight at the cause. 2) Confirm L1-L3 underneath: interface up, and for IPv6 that the link-local is present and NOT in DUPLICATE state (a DAD failure on fe80:: silently kills OSPFv3/EIGRPv6 adjacency); for BGP, ping the peer\'s IPv6 address and confirm a route to it exists. 3) For OSPFv3 stuck in EXSTART/Exchange, check MTU on both ends first (the highest-yield cause), then reconcile hello/dead intervals, area ID, area type, and authentication; verify no ACL is dropping IP protocol 89. 4) For BGP stuck in Active/Idle, test TCP/179 reachability (telnet to 179) to separate an ACL/firewall/CoPP block from a routing problem, then verify remote-AS, \'update-source\' (loopback peering), \'ebgp-multihop\'/TTL for multihop eBGP, and any MD5/TCP-AO password. 5) Fix the one mismatched attribute, watch the neighbor walk to FULL / Established, and confirm the expected prefixes are now installed before clearing the cutover gate. Use \'debug ipv6 ospf adj\'/\'debug ipv6 ospf hello\' or \'debug bgp ipv6 unicast\' only to confirm a hypothesis, never as the first step on a production box.", "alternatives": "The cheaper, riskier shortcuts are to bounce the neighbor (\'clear bgp ipv6 unicast *\' or \'clear ipv6 ospf process\') hoping it re-converges, to mask an MTU mismatch with \'ipv6 ospf mtu-ignore\' so EXSTART clears without fixing the underlying MTU, or to widen an ACL / loosen authentication to get the session up. Each can flip the adjacency green in seconds, but it ships the real defect into production \\u2014 mtu-ignore lets oversized LSAs be dropped and corrupts the database, a flap re-converges onto the same broken state, and a loosened ACL/auth is a posture regression.", "tradeoffs": "Fixing the actual mismatched parameter costs a diagnostic pass and coordination with the peer\'s owner, but it yields an adjacency that stays up and a route table that is genuinely complete at cutover; the bounce-and-mask alternatives buy an immediately green status at the price of a latent black-hole, a corrupted LSDB, or a weakened control-plane that surfaces under load after the window closes.", "citation": "Cisco, \'Troubleshoot OSPF Neighbor Problems\' (Doc ID 13699) and \'Troubleshoot OSPF Neighbors Stuck in Exstart/Exchange State\' (Doc ID 13684), plus \'Troubleshoot BGP Neighborship Connection Issues\' (Doc ID 13752); RFC 5340 (OSPFv3) for link-local-sourced adjacency", "engine_actionable": true}, {"id": "bfd-session-down-failover-degraded", "title": "Restore the down BFD session before baselining \\u2014 its protected protocol has lost sub-second failure detection", "domain": "routing", "priority": "High", "design_intent": "Bidirectional Forwarding Detection is a lightweight, protocol-independent hello mechanism (UDP/3784) whose only job is to detect, in tens of milliseconds, that the forwarding path between two adjacent engines has failed, and then notify the routing protocols registered as its clients (BGP, OSPF, EIGRP, IS-IS, HSRP/static). Per RFC 5880, BFD itself takes no corrective action \\u2014 it merely declares the session Down (diagnostic \'Control Detection Time Expired\', \'Echo Function Failed\', or \'Neighbor Signaled Session Down\') and hands that event to its clients, which then tear down the adjacency and reconverge. So a Down BFD session does not by itself drop traffic, but it silently removes the fast trigger: the protected protocol falls back to its own slow native timers (e.g. OSPF dead-interval of 40s, BGP hold-time of 180s), turning a sub-second failover into seconds-to-minutes of black-holing. That is precisely the wrong state to carry into a cutover, where you are deliberately failing links and nodes and depending on fast reconvergence to keep the outage inside the maintenance window; a down BFD session means your stated convergence SLA is not actually being met, and the post-cutover diff will not catch it because the protocol still looks \'up\'.", "observable": "The bfd axis records each BFD session state (parse_bfd_neighbors / show bfd neighbors).", "trigger": "snap[\'bfd\'][host].sessions has a session in a Down state.", "recommended_action": "Work the session from the bottom of the stack up. (1) Confirm the underlying L1/L2 path and IP reachability first \\u2014 `show bfd neighbors [details]` for the session\'s last-down diagnostic, plus `show ip interface brief` / line-protocol on the interface; a flapping or down interface is the usual root cause. (2) If the link is up, rule out a configuration asymmetry: BFD must be enabled on BOTH peers, and the operating mode (asynchronous vs. echo) and the negotiated tx/rx/multiplier timers must be compatible \\u2014 a one-sided \'bfd interval\' or an echo-mode mismatch leaves the session stuck Down. (3) Confirm the protocol client is actually registered (e.g. `bfd` under the BGP neighbor / `bfd all-interfaces` under OSPF) \\u2014 BFD with no client does nothing. (4) Verify nothing is dropping the BFD control packets on the path: an interface ACL that permits BFD but carries the `log` keyword, or that misses UDP/3784 (and the echo-source for multihop), will break the session even though data forwards; check CoPP on the supervisor for BFD-class drops too. (5) Only once the session reaches Up with stable timers do you re-baseline and treat fast-failover as available for the gate.", "alternatives": "The cheaper path is to proceed without BFD and rely on the routing protocol\'s native fast-timers (aggressive OSPF/BGP hello/hold), or to simply note the session down and cut over anyway, planning to fix it post-migration.", "tradeoffs": "Aggressive native timers cost CPU and risk false-positive flaps under load and have a detection floor far above BFD\'s, so cutting over with BFD down trades a few minutes of diagnosis now for a real risk of a multi-second (or longer) black-hole during the very failover the change is designed to exercise.", "citation": "IETF RFC 5880, Bidirectional Forwarding Detection (BFD) \\u2014 session states (Down/Init/Up) and diagnostic codes (Control Detection Time Expired / Echo Function Failed / Neighbor Signaled Session Down); BFD reports state change to registered clients which take action. Corroborated by Cisco \'Troubleshoot Bidirectional Forwarding Detection in Cisco IOS XE\' (Doc ID 220364) \\u2014 UDP/3784, async vs echo, ACL-log breakage, `show bfd neighbors details`.", "engine_actionable": true}, {"id": "copp-control-plane-policer-dropping", "title": "Identify which CoPP class is dropping and why before the cutover \\u2014 the policer is shielding the supervisor, not failing", "domain": "management", "priority": "High", "design_intent": "Control-Plane Policing applies a hardware QoS policy to traffic punted to the route-processor/supervisor, rate-limiting each protocol class (BGP, OSPF, ARP/glean, ICMP, LISP, BFD, management, etc.) so that a flood \\u2014 malicious or accidental \\u2014 cannot starve the CPU and collapse the control plane. Non-zero drops are therefore an expected, healthy signal in many cases: the policer is doing its job. But \'CoPP is dropping\' is genuinely ambiguous and must be triaged, because it can mean one of three very different things: (a) a DoS/scan or a misbehaving host is over-driving a class (the policer is protecting you), (b) a class\'s committed rate is set too low for this network\'s legitimate load (the policer is now actively harming you by clipping real BGP/ARP/LISP/BFD packets), or (c) a transient burst during convergence. In a brownfield migration this matters because cutover events generate large, legitimate bursts of exactly the punted traffic CoPP guards \\u2014 ARP/ND storms as endpoints re-resolve gateways, BGP/OSPF re-advertisement, BFD churn, IGMP \\u2014 and a default or under-sized policer can drop those protocol packets at the worst possible moment, manifesting as slow reconvergence or stranded endpoints that look like a routing bug. Carrying unexplained CoPP drops into the gate means you cannot distinguish \'protected\' from \'self-inflicted clipping\'.", "observable": "The copp axis records per-class control-plane policer drops/exceeded/violated (parse_copp_drops / show policy-map control-plane).", "trigger": "snap[\'copp\'][host] has a class with non-zero drops.", "recommended_action": "Attribute, then decide \\u2014 never blindly raise rates. (1) Pin down which class and how hard: on NX-OS `show policy-map interface control-plane` exposes per-class conformed vs. violated/dropped bytes plus the 5-minute violate rate and violated-peak timestamp; correlate the spike with a change/event window. (2) Classify the drop: map the class back to its CoPP ACL and confirm whether the source is legitimate control traffic at volume or anomalous (a single talker, a scan, a loop/glean storm \\u2014 for glean specifically, fix next-hop reachability and enable `hardware ip glean throttle` rather than widening the policer). (3) If the traffic is legitimate and the rate is simply too small for this fleet, tune that specific class\'s CIR/BC upward from the appropriate default profile (strict/moderate/lenient/dense) \\u2014 Cisco\'s guidance is to start from the default \'strict\' policy and then customize per the protocols and scale actually present, treating CoPP as an ongoing baseline that must be revisited whenever devices/services are added (i.e. at every migration). (4) Re-measure to confirm drops for that class fall to zero/transient under normal load before declaring the control plane clean for the gate.", "alternatives": "The fast options are to leave the default policy untouched and accept the drops, or to swap to a more permissive profile (moderate/lenient) or bump every class\'s rate wholesale so the violations disappear from the dashboard.", "tradeoffs": "Blanket-loosening CoPP makes the alarm go away but reopens the exact CPU-exhaustion attack surface CoPP exists to close, so the disciplined move is to widen only the one legitimately-saturated class by a measured amount \\u2014 keeping protection intact while ensuring real BGP/ARP/LISP/BFD control traffic survives the cutover burst.", "citation": "Cisco \'Verify Control Plane Policing Violations on Nexus Platforms\' (Doc ID 217946) \\u2014 `show policy-map interface control-plane` violated/dropped-bytes and 5-min violate-rate interpretation, and triage of drops as attack vs. misconfiguration; with Cisco Nexus 9000 NX-OS Security Configuration Guide (Configure CoPP) \\u2014 default \'strict\' start point and per-class CIR/BC tuning as an ongoing process; and the Cisco \'Control Plane Policing Implementation Best Practices\' note.", "engine_actionable": true}, {"id": "cts-environment-data-not-downloaded", "title": "Re-establish the TrustSec environment-data download before cutover \\u2014 without it the device has no SGT/SGACL policy to enforce", "domain": "security", "priority": "High", "design_intent": "In a Cisco TrustSec / SD-Access segmentation design, the enforcing switch does not store its policy locally; it downloads \'environment data\' from ISE \\u2014 the Security Group Tag name table, the SGACL/policy server list, the device\'s own SGT, and the refresh timers \\u2014 and only then can it map a tag to an SGACL and enforce group-based segmentation. When the environment-data download fails (engine emits this on a `show cts environment-data` state that is not COMPLETE / last status not Successful), the device is left with stale or empty policy: it cannot resolve SGTs to SGACLs, so the intended micro-segmentation either is not applied (an open-policy gap) or, worse, an in-flight refresh that fails can clear the policies it had, dropping legitimate traffic. For a brownfield migration this is a security-posture gate, not a cosmetic one: if you cut endpoints onto a fabric whose enforcement nodes have no environment data, you are silently running the network unsegmented \\u2014 east-west controls you certified in design are simply absent \\u2014 and the failure is easy to miss because forwarding still works. The download is also fragile in exactly the conditions a migration creates (ISE reachability, new NADs, credential/PAC provisioning), so it must be proven green per node before the segmentation gate is signed.", "observable": "The cts axis records the TrustSec environment-data download state (parse_cts_environment_data / show cts environment-data).", "trigger": "snap[\'cts\'][host].environment_data.state is not complete/downloaded.", "recommended_action": "Restore the device\\u2192ISE policy channel, then re-pull data \\u2014 carefully. (1) Read the current state with `show cts environment-data` (and the server list / last-download status) to see whether it never downloaded, is stale, or marked all servers dead. (2) Verify the transport and trust: ISE reachability (the REST/ERS path is TCP/9063), TLS handshake success, correct server IP/FQDN (a wrong or unresolvable domain name is a common cause), and that the device\'s CTS credentials/PAC match what ISE has under the network device\'s Advanced TrustSec settings \\u2014 and that ISE permits REST API calls from this NAD\'s NAS-IP. (3) Confirm server-selection health: TrustSec prefers a configured static IP, falls back to the domain name, marks a non-responding server dead, fails the private list over to the public list, and only retries on the next request after marking servers alive \\u2014 so a single dead-server entry can stall the whole pull. (4) Trigger/allow a refresh and confirm state returns to COMPLETE with the expected SGT table populated. Critical ordering caveat: do NOT manually refresh environment data while ISE is unreachable and the 802.1X links are in critical-authentication mode \\u2014 a refresh that fails can wipe the existing policies; fix reachability first, then refresh. Only a COMPLETE download with the expected SGTs clears the segmentation gate.", "alternatives": "The cheaper path is to proceed with whatever cached/stale environment data the device still holds, or to lean on the configured fallback/critical-authentication SGT and cut over, deferring the ISE fix to after the window.", "tradeoffs": "Cutting over on stale or fallback policy means the segmentation you designed and certified is not actually enforced for those endpoints \\u2014 an invisible east-west exposure \\u2014 so the safe move is to verify a live COMPLETE download per enforcement node first, since fixing ISE reachability/credentials pre-cutover is cheap compared with discovering an unsegmented fabric in production.", "citation": "Cisco TrustSec Configuration Guide (Catalyst 9000) \\u2014 \'SGACL and Environment Data Download over REST\': environment-data contents and server failover (static-IP vs domain, private\\u2192public list, mark-dead/retry), TLS-handshake/HTTP-timeout/wrong-domain failure causes, ERS port 9063, and ISE NAD/NAS-IP REST prerequisites; with the Cisco TrustSec Troubleshooting Guide \\u2014 `show cts environment-data` state=COMPLETE/Successful and the caution against refreshing env-data while ISE is unreachable in critical-auth mode (policies may clear).", "engine_actionable": true}, {"id": "lisp-fabric-session-down", "title": "Confirm whether the down LISP session is a real control-plane outage or an expected no-EID node before gating the fabric", "domain": "campus-sda", "priority": "High", "design_intent": "In an SD-Access / LISP fabric the control-plane node runs the LISP Map-Server / Map-Resolver, the authoritative database that maps every endpoint identifier (EID) to the routing locator (RLOC) of the fabric edge it lives behind. Every fabric device (edge, border, control-plane) maintains a TCP-based LISP session to each control-plane node and uses it to register the EIDs it owns and to resolve EIDs it needs to reach. When `show lisp session` to a map-server is Down, that registration-and-resolution channel is broken: edges cannot register newly-learned hosts, map-resolvers cannot answer map-requests, and traffic to not-yet-cached destinations is punted or dropped until the session and the underlay recover. In a migration this gates the fabric because cutover is the moment of maximum EID churn \\u2014 endpoints moving onto the fabric must register, and reachability to them depends entirely on a healthy session to the control plane; a down session means new and moved hosts can be black-holed even though existing cached flows look fine. The honest subtlety the engine cannot resolve alone, and which the senior engineer must: a LISP session shown Down is sometimes entirely expected \\u2014 a device with no EIDs to register (a border node that does not import routes into the fabric, or an edge with no endpoints attached) legitimately holds no/down session, so \'Down\' here is not automatically a fault. The doctrine is to disambiguate before alarming, and to never sign the fabric gate on an unverified down session to a control-plane node that should be carrying registrations.", "observable": "The lisp axis records per-VRF configured-vs-established LISP control sessions (parse_lisp_sessions / show lisp session).", "trigger": "snap[\'lisp\'][host].sessions has a VRF with total>=1 but established==0 (a partition, not a benign lone-down peer).", "recommended_action": "Disambiguate, then chase the underlay. (1) Establish whether this session SHOULD be up: identify the peer\'s role \\u2014 a session from an active edge (with endpoints) or a route-importing border to a control-plane node must be Established; a down session from a device with genuinely no EIDs to register is expected and is not a defect. (2) For a session that should be up, inspect it with `show lisp session` and `show lisp session vrf <vrf> <map-server-ip>` (state, message in/out counts, active instances) to see whether it never came up or flapped. (3) Verify the underlay first \\u2014 the LISP session is a TCP session, so confirm IP reachability/routing to the map-server RLOC and that no ACL/firewall is blocking the LISP control ports; an underlay routing or MTU problem is the common root. (4) Then check the LISP control-plane configuration symmetry on both ends: the map-server\'s site/EID configuration and `passive-open`, a complete locator-set, and matching authentication keys (a PSK mismatch fails registration for specific EIDs while the session may still form). (5) Once the session is Established and the expected EIDs are registered (verify on the control-plane node), the fabric mapping is healthy and the gate can proceed.", "alternatives": "The cheaper path is to treat any Down LISP session as benign \'no-EID\' noise and move on, or to cut over relying on the redundant/second control-plane node and existing map-cache, fixing the down session afterward.", "tradeoffs": "Assuming-benign without checking the peer\'s role risks shipping a genuine map-server outage into the cutover (new/moved endpoints silently black-holed), so the small cost of confirming role + reachability + registration up front is what separates a true non-issue from an unverified fabric-wide reachability gap.", "citation": "Cisco SD-Access Fabric Troubleshooting Guide \\u2014 control-plane node = LISP Map-Server/Map-Resolver, TCP-based `show lisp session` registration channel, and the note that a Down session is expected on devices with no EIDs to register (non-route-importing border / edge with no endpoints); with Cisco \'Troubleshoot LISP VXLAN Fabric on Catalyst 9000 Series Switches\' (Doc ID 220361) for `show lisp session vrf <ip>` detail and registration/passive-open/locator-set/key checks.", "engine_actionable": true}, {"id": "dmvpn-tunnel-peer-down", "title": "Restore the down DMVPN/NHRP spoke before treating the overlay topology as complete", "domain": "wan", "priority": "High", "design_intent": "A DMVPN spoke whose mGRE tunnel is down means NHRP next-hop resolution has failed for that site: the spoke is not registered with its Next-Hop Server (the hub), so the hub holds no mapping of the spoke\'s overlay-to-NBMA address and no dynamic spoke-to-spoke shortcut to that site can ever be built. Because DMVPN\'s bring-up is a strict dependency chain (tunnel line-up triggers NHRP registration, which in turn triggers the protected IPsec/IKE session), a peer stuck in the NHRP/down state usually points either at a broken registration (the spoke\'s WAN IP changed via DHCP/dialer, a new NAT device was inserted in the underlay, an underlay routing or ACL block of the mGRE/IPsec transport) or at the IPsec layer beneath it failing to come up. Operationally this site is partitioned from the overlay: its routes are absent from the hub\'s routing table and the branch is reachable only by whatever backup path exists. In a brownfield assessment this is a hard gate item \\u2014 a down peer is an active outage or a latent one masked by redundancy, and freezing a migration baseline while one of the overlay\'s branches is unresolved would carry an unknown, unhealed fault straight through cutover.", "observable": "The dmvpn axis records each NHRP tunnel peer state (parse_dmvpn_peers / show dmvpn).", "trigger": "snap[\'dmvpn\'][host].peers has a spoke/tunnel peer in a non-up state.", "recommended_action": "Work the dependency chain bottom-up, exactly as Cisco\'s DMVPN troubleshooting method prescribes \\u2014 verify GRE/NHRP before blaming IPsec. (1) On the hub, run \'show dmvpn detail\' and \'show ip nhrp nhs detail\' to confirm whether the spoke is registered and in which state (UP vs NHRP/IKE); a peer that never reaches UP is a registration failure, one that flaps points at holdtime/keepalive or an unstable underlay. (2) Confirm underlay reachability of the spoke\'s NBMA address and that IKE/IPsec transport (UDP 500/4500, ESP) is permitted end-to-end \\u2014 a NAT device newly in path or an ACL on the transport is the classic cause. (3) Validate the GRE/tunnel layer in isolation (tunnel source/destination, NHRP network-id, authentication, and the NHS address the spoke is configured to register to); Cisco\'s guidance is to prove the GRE tunnel works before layering IPsec. (4) Only then descend into the crypto session (see ipsec-crypto-session-down) if NHRP registration is initiating but the protected session will not establish. (5) Re-verify with \'show dmvpn detail\' that the peer reaches UP and that NHRP shortcuts re-form before clearing the finding.", "alternatives": "The cheaper path is to accept the spoke as \'down but covered\' \\u2014 rely on a backup NHS or an alternate WAN/MPLS path and proceed, deferring the fix \\u2014 or to bounce the tunnel interface to force a fresh NHRP registration without finding the root cause.", "tradeoffs": "Deferring saves a maintenance touch now but carries an unresolved single-site fault (and an unverified spoke-to-spoke data plane) into the cutover baseline, so a benign-looking redundancy event during the window can silently isolate the branch.", "citation": "Cisco \\u2014 Troubleshoot Common DMVPN Issues (NHRP registration/state, \'verify the GRE tunnels work fine without IPsec encryption\', show dmvpn detail / show ip nhrp nhs detail)", "engine_actionable": true}, {"id": "ipsec-crypto-session-down", "title": "Re-establish the down IPsec/IKE session before baselining any traffic that depends on the encrypted path", "domain": "wan", "priority": "High", "design_intent": "A down IPsec crypto session means the security association that protects this tunnel is not established, so interesting traffic destined for the peer is either black-holed or forced onto an unprotected/alternate path \\u2014 it is not being encrypted and forwarded. IPsec has two layers: IKE/ISAKMP Phase 1 (the management SA \\u2014 healthy state MM_ACTIVE for IKEv1) builds the secure channel, and Phase 2 (the IPsec SAs \\u2014 IKEv1 reaches QM_IDLE) builds the per-flow encryption SAs that actually carry data. A session can be \'down\' at either layer, and the layer matters: no Phase 1 usually means peer reachability or a Phase 1 policy/pre-shared-key/peer-identity problem; Phase 1 up but no Phase 2 usually means a transform-set incompatibility or non-mirrored crypto ACLs producing a proxy-ID (\'invalid proxy IDs\') mismatch, or a lifetime/rekey failure. Critically, an SA showing \'active\' is not proof of forwarding \\u2014 only incrementing encaps/decaps counters prove bidirectional traffic. For a brownfield migration this is a cutover gate because any branch, partner, or segment that reaches the rest of the estate over this tunnel is currently disconnected or insecurely connected; carrying a down or asymmetric crypto session into cutover means the post-change validation can pass on control-plane state while user traffic still fails to cross the protected boundary.", "observable": "The crypto axis records each IPsec/IKE crypto session status (parse_crypto_sessions / show crypto session).", "trigger": "snap[\'crypto\'][host].sessions has a session status Down.", "recommended_action": "Diagnose top of the stack down, isolating the failing phase before changing config. (1) \'show crypto session [detail]\' for a single per-peer view of session status and whether SAs are active; (2) \'show crypto isakmp sa\' (IKEv1) / \'show crypto ikev2 sa\' to confirm Phase 1 \\u2014 if it is not MM_ACTIVE/established the problem is Phase 1: verify the peer is reachable and UDP 500/4500 + ESP are permitted end-to-end, then check matching Phase 1 policy (encryption/hash/DH/lifetime), the pre-shared key, and peer identity/PKI; (3) if Phase 1 is up but Phase 2 is not, \'show crypto ipsec sa\' and confirm the transform sets match exactly and the crypto ACLs mirror each other on both peers (non-reversible ACLs cause the proxy-ID mismatch that blocks Phase 2); (4) prove the data plane by checking that encaps and decaps both increment \\u2014 increment in only one direction indicates an asymmetric/return-path or one-sided ACL fault; (5) where state is stale or flapping, \'clear crypto isakmp\' / \'clear crypto sa\' to force a clean renegotiation, using debug only when the static checks do not localize the fault. Re-verify encaps/decaps before clearing the finding.", "alternatives": "Cheaper options are to clear/bounce the SAs to force a renegotiation and hope the session re-forms without finding the underlying mismatch, or to route the affected traffic over an alternate (possibly unencrypted) path and proceed.", "tradeoffs": "A blind clear may restore the tunnel transiently but leaves a latent policy/ACL/lifetime mismatch that re-breaks at the next rekey or under load, and an unencrypted detour trades availability for a confidentiality exposure across the migration window.", "citation": "Cisco \\u2014 Understand and Use Debug Commands to Troubleshoot IPsec (IKE Phase 1/Phase 2 states, proxy-ID/transform/crypto-ACL mismatch, clear crypto, encaps/decaps verification)", "engine_actionable": true}, {"id": "storm-control-action-on-edge", "title": "Confirm the storm-control action and recovery policy on edge ports before cutover, so a storm fails predictably", "domain": "dc-switching", "priority": "Medium", "design_intent": "Storm control on an edge port meters broadcast, multicast, or unknown-unicast against a configured suppression level (percent of port bandwidth, bps, or pps) and, when the rising threshold is crossed, takes the configured action. An action of \'shutdown\' err-disables the port the moment a storm is detected \\u2014 the port stays down until it is manually bounced (shut/no shut) or auto-recovered by \'errdisable recovery cause storm-control\'; an action of \'trap\' raises an SNMP notification but leaves the port forwarding (still filtering the offending class). This is a deliberate, protective design at the access edge: it contains a loop or a misbehaving/compromised host before broadcast radiation saturates the VLAN. The reason it gates a cutover is the recovery posture, not the threshold alone: a shutdown action with no errdisable auto-recovery turns any transient storm \\u2014 exactly the kind a topology change, a re-cabling, or an STP reconvergence during the maintenance window can induce \\u2014 into a hard port-down that requires a human to clear, potentially black-holing an endpoint or an entire access closet at the worst moment. A trap-only action conversely means a real storm will be logged but not contained, so the blast radius is bounded only by the suppression level. The engine flags the configured action so the team consciously decides which failure mode each edge port should exhibit during change.", "observable": "The storm_control axis records per-port configured rules and their action (parse_storm_control / show storm-control).", "trigger": "snap[\'storm_control\'][host] has a CONFIGURED rule whose action is None (drops a storm silently).", "recommended_action": "(1) Inventory the per-port action and threshold (\'show storm-control\', \'show running-config interface\') and reconcile to intent \\u2014 access/host edge ports generally warrant tight broadcast suppression (commonly low single-digit percent to cover ARP) with a containment action, uplinks/trunks generally should not carry an aggressive shutdown that could isolate a whole downstream tree. (2) Decide the recovery model explicitly: if \'shutdown\' is used, pair it with \'errdisable recovery cause storm-control\' and a sane recovery interval so a transient storm self-heals; if a port must never auto-restore (a known-abusive segment), keep manual recovery but document the runbook step. (3) Check \'show errdisable recovery\' to confirm the cause is actually enabled and the interval is set. (4) Before the window, identify which edge ports could be transiently storm-tripped by the planned change and pre-stage the recovery (or temporarily relax to \'trap\') so the change itself does not err-disable production access. (5) Re-baseline after cutover that no port sits err-disabled by storm-control.", "alternatives": "Cheaper alternatives are to leave storm-control at trap-only (visibility without disruption) or to disable the shutdown action for the duration of the change so no port can be err-disabled by a migration-induced transient.", "tradeoffs": "Trap-only or disabling the action removes the risk of self-inflicted port-downs during the window but also removes the automatic loop/storm containment exactly when re-cabling and reconvergence make a transient loop most likely \\u2014 you trade guaranteed availability of a port against bounded blast radius if a real storm occurs.", "citation": "Cisco \\u2014 Configuring Port-Based Traffic Control: Storm Control (action shutdown=err-disable / trap; recover via shut-no shut or \'errdisable recovery cause storm-control\')", "engine_actionable": true}, {"id": "storm-control-active-suppression", "title": "Treat active storm-control suppression as a live broadcast storm and find its source before cutover", "domain": "dc-switching", "priority": "High", "design_intent": "Active suppression is not a configuration observation \\u2014 it is a runtime event: the port has crossed its rising threshold and storm control is dropping the offending traffic class (broadcast/multicast/unknown-unicast) right now, and on a per-interval basis once the level is exceeded all matching traffic is discarded for the remainder of the measurement interval (the ~1-second window on Catalyst IOS, the sub-millisecond hardware interval on Nexus). It uses rising/falling thresholds: traffic of that class is blocked above the rising level and only restored once it falls back below the falling level. An interface in active suppression therefore means real, sustained Layer-2 flooding is occurring \\u2014 most commonly a bridging loop (STP failed/edge port without BPDU guard), a NIC/teaming fault, a misconfigured multicast source, or a host under attack \\u2014 and the switch is the last line of defense holding the VLAN together by dropping packets. This is one of the most consequential conditions to carry into a migration: a loop or flood storm present at baseline will corrupt the assessment\'s view of normal traffic, will be amplified or relocated by re-cabling and topology changes during cutover, and the suppression itself is silently dropping legitimate broadcast/multicast (ARP, ND, DHCP, IGMP) for every host on that segment. A cutover must not proceed over an unresolved active storm \\u2014 the change is far more likely to turn a contained storm into an uncontained one.", "observable": "The storm_control axis records per-port filter state (parse_storm_control / show storm-control).", "trigger": "snap[\'storm_control\'][host] has a port whose filter_state is Blocking (actively suppressing a live storm).", "recommended_action": "(1) Confirm and scope the live event: \'show storm-control\' and interface counters to identify the affected port(s), the traffic class being suppressed, and whether it is sustained or bursty; correlate with the syslog storm-control messages for onset time. (2) Hunt the source as a loop first \\u2014 verify STP topology and root, check for ports that should have BPDU guard/portfast but do not, and look for two access ports bridging the same segment; a sudden multi-port broadcast spike is a loop until proven otherwise. (3) If not a loop, localize the talker: identify the MAC/host generating the flood (mis-teamed NIC, chatty multicast app, malware) and isolate or remediate it at source. (4) Validate that the suppression clears (traffic falls below the falling threshold and the port resumes forwarding the class) and that dependent broadcast/multicast services on the VLAN recover. (5) Only after the storm source is eliminated and suppression has cleared should the segment be considered cutover-ready; do not merely raise the threshold to silence the alarm.", "alternatives": "The cheaper, riskier alternative is to suppress the symptom \\u2014 raise the storm-control level (or move the action to trap) so the interface stops actively blocking \\u2014 and proceed, deferring root-cause to after the migration.", "tradeoffs": "Raising the threshold restores forwarding immediately and unblocks the segment, but it removes the only thing currently containing the storm; if the underlying loop or flood is still present, the next topology change can let it saturate the broadcast domain and take down everything on the VLAN.", "citation": "Cisco \\u2014 Cisco Nexus 9000 Series NX-OS Security Configuration Guide: Configuring Traffic Storm Control (rising/falling thresholds; once the level is reached, traffic is dropped until the interval ends)", "engine_actionable": true}, {"id": "qos-runtime-egress-queue-drops", "title": "Characterize egress queue drops and prove they spare sensitive classes before freezing the QoS baseline", "domain": "qos", "priority": "High", "design_intent": "Runtime egress (output) queue drops mean a transmit queue overflowed and the switch discarded packets \\u2014 either by tail drop / weighted tail drop when the queue (or its queue-limit) filled, or by WRED selectively dropping as the queue approached congestion. Output drops are normal and often benign for elastic TCP traffic; what matters is which queue is dropping and what class lives in it. They arise from three structural causes: a speed mismatch / oversubscription (a faster source into a slower egress, or many ingress ports converging on one uplink), microbursts (sub-second traffic spikes that exceed the port\'s buffer even though the average rate is low), and under-provisioned or mis-tuned buffers/queue-limits. The migration risk is twofold. First, drops in a priority or low-latency queue (voice, video, storage, control/keepalive traffic) directly degrade those services and can be the hidden cause of call quality, application timeouts, or protocol flaps \\u2014 and a cutover that re-homes traffic onto a new uplink or collapses several links into one can sharply increase oversubscription and microburst exposure, turning tolerable drops into service-affecting ones. Second, if the baseline is frozen without characterizing the drops, the post-cutover comparison cannot distinguish \'pre-existing benign TCP drops\' from \'new drops we just introduced on the voice queue\', so the change\'s true impact on sensitive classes is invisible.", "observable": "The qos_runtime axis records per-class egress drop/police counters (parse_policymap_drops / show policy-map interface).", "trigger": "snap[\'qos_runtime\'][host] has a priority/LLQ class congestion-dropping, or a data class over threshold.", "recommended_action": "Diagnose by isolating the queue, not by globally widening buffers. (1) Confirm and rate the drops with \'show interfaces <intf>\' (total output drops and trend) \\u2014 a tiny, stable count on a best-effort class may be acceptable; a growing count or any count on a priority class is not. (2) On Catalyst 9000, attribute drops to a specific queue with \'show platform hardware fed switch active qos queue stats interface <intf>\' and inspect the buffer allocation with \'show platform hardware fed switch active qos queue config interface <intf>\' (Hardmax/Softmax); on routers/MQC platforms use \'show policy-map interface <intf>\' to see per-class queue and drop counters. (3) Determine the cause class \\u2014 sustained oversubscription (a capacity/topology fix), microbursts (a buffer/queue-limit fix), or a queueing policy that starves a class. (4) For microbursts, raise the shared-buffer headroom first as the low-risk mitigation (\'qos queue-softmax-multiplier\', default 100, up to the platform maximum) and/or tune the specific queue-limit; for a class being starved, correct the egress queuing/bandwidth policy so priority traffic is protected. (5) For genuine oversubscription, the real fix is capacity (more/faster uplinks, load-balancing, or moving the talker) \\u2014 buffers only defer the loss. Re-verify per-queue stats after the change and confirm sensitive classes show zero drops.", "alternatives": "The cheaper alternatives are to accept the drops as normal best-effort behavior and move on (valid only once you have proven no priority/latency-sensitive class is affected), or to bump the softmax buffer multiplier globally as a blanket absorber without identifying the offending queue.", "tradeoffs": "Accepting or buffering-over the drops avoids a QoS redesign and a capacity spend now, but deeper buffers add latency/jitter and only mask microbursts \\u2014 for true oversubscription the loss is merely deferred, so an uncharacterized drop on a voice/control queue can surface as a service outage after cutover concentrates more traffic on the link.", "citation": "Cisco \\u2014 Troubleshoot Output Drops on Catalyst 9000 Switches (per-queue \'show platform hardware fed ... qos queue stats/config\', Hardmax/Softmax buffers, microburst mitigation via qos queue-softmax-multiplier)", "engine_actionable": true}]')
