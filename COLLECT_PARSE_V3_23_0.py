...
from typing import Dict, List, Optional   # Tuple dropped step 27 (the ARP/dual-connection builders moved)
...
from cisco_toolkit.textutils import (
    # PHYSICAL_IFACE_RE dropped step 27 (build_device_physical, its last monolith user, moved to build);
    # _TRUNK_STATUS_WORDS dropped step 26; VALID_IFACE_RE / IFACE_TOKEN_RE / normalize_speed earlier (17/25).
    normalize_ifname, detect_link_type, safe_fs_name,
)
...
from cisco_toolkit.parse import (   # NEW-V3.23.12-.15 (PHASE 2.7 steps 2-5): primitives + pure parsers
    ...
    # step 27 dropped parse_show_ip_arp / parse_vtp_status / parse_switch_mgmt_ip /
    # parse_show_version / parse_show_inventory / parse_show_environment_power /
    # parse_show_environment / parse_show_module_count: their last monolith users
    # (build_device_physical / build_switch_identity / collect_global_arp) moved to build.py.
    # step 25 dropped parse_show_interface_counters / _parse_fhrp / _is_physical_port /
    # _classify_media / parse_interface_phy / _parse_poe_watts: their last monolith users
    # (write_physical_health_sheet / write_l3_forwarding_sheet) moved to excel.
)
...
# NEW-V3.23.37 (PHASE 2.7 step 27): the model-construction layer - build switch-level
# DevicePhysical / switch-identity records + global-ARP enrichment from collected output.
# Homed in cisco_toolkit/build.py; imported back so main() keeps calling them. (build_interfaces,
# the big per-device InterfaceData builder, stays in this file for now - a later step.)
