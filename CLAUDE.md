...
- GitHub Support ticket `#4624412` — server-side purge of pre-rewrite objects and stale
  `refs/pull/*`. Until they act, superseded SHAs may still resolve by direct URL. The repository is
  intentionally **PUBLIC** as of the 2026-09-05 Release-1 review; do not rely on repository privacy.
...
- For a relocatable generated Obsidian vault, use
  `py -3.12 -I -B tools/export_graphify_obsidian.py --graph <graph.json> --labels <labels.json> --dir <fresh-vault>`.
  It pins the reviewed Graphify exporter sources, derives node filenames from stable IDs rather than the
  absolute output root, builds in isolation, preflights the complete target member set, and publishes only
  into a fresh vault. Never export directly into the personal vault.
...
- **Atlas — the portable field app** (`portable/`, ADR-0004): the whole platform is built as a one-folder Windows bundle designed to run from USB without host-installed Python. `Atlas.exe` is the ONE door — it serves AssessHub *and* is the engine CLI via the `--run-engine <engine argv>` sentinel (there is no separate `cisco-assess.exe` on the stick). Release candidate: `python -m portable.build_release --output <fresh external dir>` under the pinned/hash-checked toolchain; focused developer build: `python portable/build_atlas.py`; install/update: `portable/make_stick.ps1 -Dest E:\ -Package <verified ZIP>`. The four-step smoke runs a copied field-layout bundle, requires every non-`data` entry to remain byte-identical, and requires bundle-root frozen engine logging under `Atlas\data`; external per-job working directories keep isolated logs. This preserves the retained build and concurrent job custody while executing the exact double-click/field working-directory path. The staged updater verifies a same-volume package/extracted/member set, preflights a database copy, detaches client `data\` while candidate code runs, and switches whole directories under a recovery journal. Field commands remain `Atlas.exe --selftest` and **`Atlas.exe --redact-folder <collection> --out <dir>` [`--redact-collection`] [`--reuse-out`]**. The redaction run independently verifies its registry-owned CLI artifact denominator and preserves explicit incomplete/refusal states; `cutover`/`nrfu` remain AssessHub-only and PIR remains conditional post-execution. Read `portable/README-FIELD.txt` before field use. Automated evidence does not replace the Python-absent disconnected-host, signing, managed-policy, physical-media, legal, peer-review, or operator-acceptance gates.
- **Tests:** `python -m pytest -q` (`tests/` + `webapp/tests` are both in the default gate; run collection rather than caching a count). **graphify here:** read with `py -3.12 -m graphify query|explain|path`; source-extract only with guarded `update`, `watch`, or `extract <local-path> --code-only` forms above. Set the release candidate version in `pyproject.toml` before its exact-source build/review and require the tag mapping to match; never bump the decoupled schema version `cisco_toolkit.__version__` merely to match it.
