...
class CompilerTests(unittest.TestCase):
    def test_bounded_curated_claim_census_is_source_bound_but_never_closes_global_gate(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(summary["expected_candidates"], 2140)
            self.assertEqual(summary["discovered_candidates"], 2140)
            self.assertEqual(summary["classified_candidates"], 2140)
            self.assertEqual(summary["unresolved_candidates"], 2140)
            ...
            self.assertEqual(len(facet_records), 2140)
            self.assertEqual(manifest["groups"]["consequential_claim_facets"]["record_count"], 2140)
            self.assertEqual(len({row["id"] for row in facet_records}), 2140)
            self.assertEqual(len({row["facet_id"] for row in facet_records}), 2140)
