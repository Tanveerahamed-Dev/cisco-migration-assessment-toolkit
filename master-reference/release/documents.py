...
<section><h2>Integrity and limits</h2><p>This HTML has no external asset, request, analytics, persistence, or runtime-AI mechanism and does not intentionally ingest or embed raw client/Vault sources. High-confidence scans found no matching credential patterns; contextual, encoded, and container review remains pending. Verify its SHA-256 through the adjacent release manifest. It remains a blocked unsigned preview; structural accounting, the complete machine projection, independent review, and publication authority are separate gates.</p></section>
