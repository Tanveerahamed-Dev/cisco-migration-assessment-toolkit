...
| 6 | **One genuinely superlinear perf term** | `compute_failure_impact` measured **23.16 s** on the real AJ fleet — O(H×V×link-scan) with an *uncached* per-call VLAN-range `re.split` (`analyze.py:885-926`, `:595`); projects to tens of minutes at ~1000 devices. (Topology rebuilt at 9 sites + network-model at 6 + `link_centrality` twice are real duplication but only ~0.13–0.16 s each — SSOT wins, not perf levers.) | Performance |
