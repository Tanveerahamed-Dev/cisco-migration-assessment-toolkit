...
#: The ONE `*.snapshot.json` that is not client evidence: the synthetic demo fleet shipped so the
#: web UI has something to render out of the box. `cisco_toolkit/distribution_verify.py` already
#: exempts it by this exact name; without the same exemption here the two gates disagree and this one
#: rejects every build. Keyed on the full archive-relative path, not the basename, so a real
#: `something.snapshot.json` cannot be smuggled in by renaming it.
_ALLOWED_SNAPSHOT_MEMBERS = frozenset({"webapp/sample_data/sample_fleet.snapshot.json"})
...
def _member_policy_errors(name: str, *, synthetic_tests_allowed: bool = False) -> list[str]:
    ...
    is_allowed_demo_fixture = name.replace("\\", "/") in _ALLOWED_SNAPSHOT_MEMBERS
    if matches_forbidden and not is_reviewable_test_artifact and not is_allowed_demo_fixture:
...
def _is_allowed_wheel_location(parts: tuple[str, ...], dist_info_root: str | None) -> bool:
    ...
    if len(parts) >= 2 and parts[:2] == ("webapp", "backend"):
        return True
    # Written against a pyproject whose `packages.find` was ["cisco_toolkit*", "webapp.backend"].
    # It has since widened to include `webapp` itself plus package-data for the built SPA and the
    # synthetic demo snapshot, so this allowlist rejected the project's own wheel: `webapp/__init__.py`,
    # `webapp/dev.py`, every `webapp/frontend/dist/**` member and `webapp/sample_data/*.json`.
    # These are not client evidence -- the SPA is a build product of TRACKED source (tracked precisely
    # so the archives bind to a commit), and the sample snapshot is the deliberately synthetic demo
    # fixture that `webapp/sample_data/build_sample.py` regenerates. Widened to match what the
    # packaging config actually declares; the FILENAME denylist below is untouched and still applies
    # to every one of these paths, so a real capture dropped into them is still rejected.
    if len(parts) >= 2 and parts[:2] == ("webapp", "frontend") and "dist" in parts[2:3]:
        return True
    if len(parts) >= 2 and parts[:2] == ("webapp", "sample_data"):
        return True
    if len(parts) == 2 and parts[0] == "webapp" and parts[1] in ("__init__.py", "dev.py"):
        return True
...
def discover_artifacts(inputs: Iterable[Path]) -> tuple[list[Path], list[str]]:
    ...
    for item in inputs:
        ...
        # The distribution PROOF is written into this directory by design -- ci.yml and release.yml
        # both pass `--json-out dist/dist-verification.json`, and release.yml re-downloads it here on
        # an idempotent re-run. Treating it as contamination made this audit reject every real
        # release directory. It is a generated attestation ABOUT these archives, not a shipped
        # artifact; it is never packaged (nothing includes `dist/` in the wheel or sdist).
        _PROOF_SIDECARS = {"dist-verification.json"}
        unexpected = sorted(path.name for path in item.iterdir()
                            if path not in expected and path.name not in _PROOF_SIDECARS)
        if len(wheels) != 1:
            errors.append(
